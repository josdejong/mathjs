...
export const createSylvester = /* #__PURE__ */ factory(name, dependencies, ({ typed, schur, matrixFromColumns, matrix, multiply, range, concat, transpose, index, subset, add, subtract, identity, lusolve, abs }) => {
  /**
   *
  ...
  return typed(name, {
    'Matrix, Matrix, Matrix': function (A, B, C) {
      if (!isMatrix(A)) { A = matrix(A) }

      if (!isMatrix(B)) { B = matrix(B) }

      if (!isMatrix(C)) { C = matrix(C) }
