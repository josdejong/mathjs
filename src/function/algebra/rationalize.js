        ...
        if (tp === 'FunctionNode') {
          ...
          if (node.op === '^') {
            ...
            if (
              ...
              !isInteger(parseFloat(node.args[1].value))
            ) {
            ...
            if (oper.indexOf(node.op) === -1) {
              throw new Error(
                'Operator ' + node.op + ' invalid in polynomial expression'
              )
            }
      ...
      function recurPol(node, noPai, o) {
        ...
        if (tp === 'FunctionNode') {
          ...
          if ('+-*^'.indexOf(node.op) === -1)
            throw new Error('Operator ' + node.op + ' invalid')

          if (noPai !== null) {
            // -(unary),^  : children of *,+,-
            if (
              (node.fn === 'unaryMinus' || node.fn === 'pow') &&
              noPai.fn !== 'add' &&
              noPai.fn !== 'subtract' &&
              noPai.fn !== 'multiply'
            ) {
