...
describe('ImmutableDenseMatrix', function () {
  ...
  describe('size', function () {
    it('should return the expected size', function () {
      ...
      assert.deepStrictEqual(
        new ImmutableDenseMatrix([[1], [2], [3]]).size(),
        [3, 1]
      )
      assert.deepStrictEqual(
        new ImmutableDenseMatrix([[[1], [2], [3]]]).size(),
      ...
      assert.deepStrictEqual(
        new ImmutableDenseMatrix([[[3]]]).size(),
        [1, 1, 1]
      )
  ...
  describe('get subset', function () {
    it('should get the right subset of the matrix', function () {
      ...
      assert.deepStrictEqual(m.subset(index(1, new Range(1, 3))).valueOf(), [
        [5, 6],
      ])
      assert.deepStrictEqual(m.subset(index(0, new Range(1, 3))).valueOf(), [
        [2, 3],
      ])
      assert.deepStrictEqual(m.subset(index(new Range(1, 3), 1)).valueOf(), [
        [5],
        [8],
      ])
      assert.deepStrictEqual(m.subset(index(new Range(1, 3), 2)).valueOf(), [
        [6],
        [9],
      ])
      ...
      assert.deepStrictEqual(m.subset(index(1, 1, new Range(0, 2))).valueOf(), [
        [[7, 8]],
      ])
      assert.deepStrictEqual(m.subset(index(1, new Range(0, 2), 1)).valueOf(), [
        [[6], [8]],
      ])
      assert.deepStrictEqual(m.subset(index(new Range(0, 2), 1, 1)).valueOf(), [
        [[4]],
        [[8]],
      ])
    })
  ...
  describe('map', function () {
    ...
    it('should invoke callback with parameters value, index, obj', function () {
      ...
      assert.deepStrictEqual(m2.toArray(), [
        ['[1,[0,0],true]', '[2,[0,1],true]', '[3,[0,2],true]'],
        ['[4,[1,0],true]', '[5,[1,1],true]', '[6,[1,2],true]'],
      ])
    })
  })
