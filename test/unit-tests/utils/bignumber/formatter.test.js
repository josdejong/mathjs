...
describe('format', function () {
  describe('format', function () {
    ...
    it('auto notation', function () {
      ...
      assert.strictEqual(format(new B(0.104)), '0.104')
      ...
      assert.strictEqual(format(new B(2.3e1)), '23')
      assert.strictEqual(format(new B(2.3e2)), '230')
      assert.strictEqual(format(new B(2.3e3)), '2300')
      assert.strictEqual(format(new B(2.3e4)), '23000')
      assert.strictEqual(format(new B(2.3e5)), '2.3e+5')
      assert.strictEqual(format(new B(2.3e6)), '2.3e+6')
      ...
      assert.strictEqual(
        format(new B('123456789123456789')),
        '1.23456789123456789e+17'
      )
      assert.strictEqual(
        format(new B('123456789123456789123456789')),
        '1.23456789123456789123456789e+26'
      )

      assert.strictEqual(format(new B(0.1111e6)), '1.111e+5')
      assert.strictEqual(format(new B(0.3333e6)), '3.333e+5')
      assert.strictEqual(format(new B(0.6666e6)), '6.666e+5')
      assert.strictEqual(format(new B(0.9999e6)), '9.999e+5')
    ...
    describe('should apply options', function () {
      it('auto notation with precision', function () {
        ...
        assert.strictEqual(format(new B(1000.0), { precision: 5 }), '1000')
        assert.strictEqual(format(new B(1000.001), { precision: 5 }), '1000') // rounded off at 5 digits
        ...
        assert.strictEqual(format(new B(3e6), { precision: 8 }), '3e+6') // should remove trailing zeros
      ...
      it('auto notation with custom lower and upper bound', function () {
        ...
        assert.strictEqual(format(new B(1e9), options), '1e+9')
        assert.strictEqual(format(new B(1e9 - 1), options), '999999999')
        assert.strictEqual(format(new B(1e-6), options), '0.000001')
        assert.strictEqual(format(new B(0.999e-6), options), '9.99e-7')
        assert.strictEqual(
          format(new B(123456789123), options),
          '1.23456789123e+11'
        )
      })
      ...
      it('auto notation with custom lower and upper bound (2)', function () {
        ...
        assert.strictEqual(
          format(new B(0.009), { lowerExp: -2, precision: 1 }),
          '9e-3'
        )
        assert.strictEqual(
          format(new B(0.0096), { lowerExp: -2, precision: 1 }),
          '0.01'
        )
        assert.strictEqual(
          format(new B(0.01), { lowerExp: -2, precision: 1 }),
          '0.01'
        )
        assert.strictEqual(
          format(new B(0.001), { lowerExp: -2, precision: 1 }),
          '1e-3'
        )
      })
    })
    ...
    it('should format bignumbers with a custom formatting function', function () {
      ...
      assert.strictEqual(
        format(new BigNumber(1.2e6), asCurrency),
        '$1200000.00'
      )
    })
    ...
    it('should format bignumbers with custom precision, lower, and upper bound', function () {
      ...
      assert.deepStrictEqual(
        format(new Big(5).div(3), options),
        '1.6666666666666666666666666666666666666666666666667'
      )
      assert.deepStrictEqual(
        format(new Big(5e40).div(3), options),
        '16666666666666666666666666666666666666666.666666667'
      )
      assert.deepStrictEqual(
        format(new Big(5e-40).div(3), options),
        '0.00000000000000000000000000000000000000016666666666666666666666666666666666666666666666667'
      )
      assert.deepStrictEqual(
        format(new Big(5e60).div(3), options),
        '1.6666666666666666666666666666666666666666666666667e+60'
      )
      assert.deepStrictEqual(
        format(new Big(5e-60).div(3), options),
        '1.6666666666666666666666666666666666666666666666667e-60'
      )
      assert.deepStrictEqual(
        format(new Big(5e-80).div(3), options),
        '1.6666666666666666666666666666666666666666666666667e-80'
      )
    })
    ...
    it('auto notation with custom upper bound', function () {
      ...
      assert.strictEqual(format(new BigNumber(1e9), options), '1e+9')
    ...
    it('should format bignumbers in fixed notation', function () {
      ...
      assert.deepStrictEqual(
        format(new BigNumber('1.23456'), options),
        '1.23456'
      )
      assert.deepStrictEqual(format(new BigNumber('1.7'), options), '1.7')
      assert.deepStrictEqual(
        format(new BigNumber('12345678'), options),
        '12345678'
      )
      assert.deepStrictEqual(
        format(new BigNumber('12e18'), options),
        '12000000000000000000'
      )
      assert.deepStrictEqual(
        format(new BigNumber('12e30'), options),
    ...
    it('should format bignumbers in fixed notation with precision', function () {
      ...
      assert.deepStrictEqual(
        format(new BigNumber('12e18'), options),
        '12000000000000000000.00'
      )
      assert.deepStrictEqual(
        format(new BigNumber('12e30'), options),
        '12000000000000000000000000000000.00'
      )
    })
  ...
  it('should format a bignumber using toExponential', function () {
    ...
    assert.strictEqual(toExponential(new Big(2.34e3)), '2.34e+3')
    assert.strictEqual(toExponential(new Big(2.34e-3)), '2.34e-3')
    assert.strictEqual(toExponential(new Big(2.34e3), 2), '2.3e+3')
    assert.strictEqual(
      toExponential(new Big(2e3), 20),
      '2.0000000000000000000e+3'
    )
    assert.strictEqual(
      toExponential(new Big(2e3), 21),
      '2.00000000000000000000e+3'
    )
    assert.strictEqual(
      toExponential(new Big(2e3), 22),
      '2.000000000000000000000e+3'
    )
    assert.strictEqual(
      toExponential(new Big(2e3), 30),
      '2.00000000000000000000000000000e+3'
    )
    assert.strictEqual(
      toExponential(new Big('2e+300'), 30),
      '2.00000000000000000000000000000e+300'
    )
    assert.strictEqual(
      toExponential(new Big('2e-300'), 30),
      '2.00000000000000000000000000000e-300'
