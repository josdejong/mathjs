...
describe('security', function () {
  ...
  it('should allow invoking methods on complex numbers', function () {
    assert.deepStrictEqual(
      math.evaluate('complex(4, 0).sqrt(2)'),
      math.complex(2, 0)
    )
  })
    ...
    Object.keys(math.expression.mathWithTransform).forEach(function (name) {
      ...
      if (typeof value === 'function') {
        assert.strictEqual(
          ...
          true,
          'only plain functions expected, constructor functions not allowed (name: "' +
            name +
            '")'
        )
      } else {
        ...
        if (value && typeof value === 'object') {
          if (isPlainObject(value)) {
            throw new Error(
              'plain objects are not allowed, only class instances (object name: ' +
                name +
                ')'
            )
          }
