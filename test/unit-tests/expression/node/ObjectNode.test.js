...
describe('ObjectNode', function () {
  ...
  it('should LaTeX an ObjectNode', function () {
    ...
    assert.strictEqual(n2.toTex(), '\\begin{Bmatrix}\\mathbf{c:} & 3\\\\\\mathbf{n1:} & \\begin{Bmatrix}\\mathbf{a:} & 1\\\\\\mathbf{b:} & 2\\\\\\end{Bmatrix}\\\\\\end{Bmatrix}')
  })
  ...
  it('should LaTeX an ObjectNode with custom toTex', function () {
    ...
    assert.strictEqual(n.toTex({ handler: customFunction }), '\\begin{Bmatrix}\\mathbf{a:} & const\\left(1, number\\right)\\\\\\mathbf{b:} & const\\left(2, number\\right)\\\\\\end{Bmatrix}')
  })
})
