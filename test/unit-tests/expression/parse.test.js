...
describe('parse', function () {
  ...
  describe('variables', function () {
    ...
    it('should parse valid accessor assignments', function () {
      ...
      parseAndEval('O.y[3] = 8', scope)
      assert.deepStrictEqual(scope.O, { x: 5, y: [6, 7, 8] })
    })
    ...
    it('should parse valid matrix assignments', function () {
      ...
      parseAndEval('[a; b] = [4; 5]', scope)
      assert.strictEqual(scope.a, 4)
      assert.strictEqual(scope.b, 5)
      parseAndEval('[a, b; c, d] = [6, 7; 8, 9]', scope)
      assert.strictEqual(scope.a, 6)
      assert.strictEqual(scope.b, 7)
      assert.strictEqual(scope.c, 8)
      assert.strictEqual(scope.d, 9)
      parseAndEval('[a, [b, [c]]] = [10, [11, [12]]]', scope)
      assert.strictEqual(scope.a, 10)
      assert.strictEqual(scope.b, 11)
      assert.strictEqual(scope.c, 12)
      parseAndEval('[a; [b; [c]]] = [13; [14; [15]]]', scope)
      assert.strictEqual(scope.a, 13)
      assert.strictEqual(scope.b, 14)
      assert.strictEqual(scope.c, 15)
      parseAndEval('B = [16; 17]', scope)
      parseAndEval('[a; B[1, 1]; B[2, 1]; c] = [18; 19; 20; 21]', scope)
      parseAndEval('[b1, b2] = [B[1, 1], B[2, 1]]', scope)
      assert.strictEqual(scope.a, 18)
      assert.strictEqual(scope.b1, 19)
      assert.strictEqual(scope.b2, 20)
      assert.strictEqual(scope.c, 21)
      parseAndEval('O = { x: 22, y: 23 }', scope)
      parseAndEval('[O.x, O.y] = [24, 25]', scope)
      assert.strictEqual(scope.O.x, 24)
      assert.strictEqual(scope.O.y, 25)
      parseAndEval('O.z = [26, 27]', scope)
      parseAndEval('[O.z[1], O.z[2]] = [28, 29]', scope)
      parseAndEval('[z1; z2] = [O.z[1]; O.z[2]]', scope)
      assert.strictEqual(scope.z1, 28)
      assert.strictEqual(scope.z2, 29)
      parseAndEval('[x, y, z] = 2 * [1, 2, 3] / 2', scope)
      assert.strictEqual(scope.x, 1)
      assert.strictEqual(scope.y, 2)
      assert.strictEqual(scope.z, 3)
      parseAndEval('[x, y, z] = 4:6', scope)
      assert.strictEqual(scope.x, 4)
      assert.strictEqual(scope.y, 5)
      assert.strictEqual(scope.z, 6)
      parseAndEval('A = 7:10', scope)
      parseAndEval('[a, b, c, d] = A', scope)
      assert.strictEqual(scope.a, 7)
