...
describe('sin', function () {
  ...
  it('should return the sine of a number', function () {
    ...
    approx.equal(sin(pi / 8), 0.38268343236509, EPSILON)
  ...
  it('should return the sine of a bignumber', function () {
    ...
    assert.deepStrictEqual(
      resultVal,
      Big(
        '0.0325518169566161584427313159942672130512044591216893328934710' +
    ...
    for (let x = -20; x < 20; x += 1) {
      approx.equal(
        bigmath2.sin(bigmath2.bignumber(x)).toNumber(),
        Math.sin(x),
        EPSILON
      )
    }
    ...
    assert.deepStrictEqual(
      bigmath2.sin(bigPi.div(8)).toString(),
      '0.38268343236509'
    )
    assert.deepStrictEqual(
      bigmath2.sin(bigPi.div(4)).toString(),
      '0.707106781186547'
    )
    assert.deepStrictEqual(bigmath2.sin(bigPi.div(2)).toString(), '1')
    assert.deepStrictEqual(
      bigmath2.sin(bigPi.times(3).div(4)).toString(),
      '0.707106781186551'
    )
    assert.ok(bigmath2.sin(bigPi).lt(1e-14))
    assert.deepStrictEqual(
      bigmath2.sin(bigPi.times(5).div(4)).toString(),
      '-0.707106781186554'
    )
    assert.deepStrictEqual(bigmath2.sin(bigPi.times(3).div(2)).toString(), '-1')
    assert.deepStrictEqual(
      bigmath2.sin(bigPi.times(7).div(4)).toString(),
      '-0.707106781186553'
    )
    ...
    assert.deepStrictEqual(
      bigmath61.sin(bigmath61.bignumber(-2)).toString(),
  ...
  it('should return the sine of a complex number', function () {
    ...
    approx.deepEqual(
      sin(complex('1+i')),
      complex(1.298457581415977, 0.634963914784736),
      EPSILON
    )
  ...
  it('should return the sine of an angle', function () {
    ...
    approx.equal(
      sin(unit(math.bignumber(45), 'deg')).toNumber(),
      0.707106781186548,
      EPSILON
    )

    approx.deepEqual(
      sin(unit(complex('1+i'), 'rad')),
      complex(1.298457581415977, 0.634963914784736),
      EPSILON
    )
  })
