...
describe('eigs', function () {
  ...
  it('follows aiao-mimo', function () {
    ...
    assert(
      Array.isArray(realSymArray.values) &&
        typeof realSymArray.values[0] === 'number'
    )
    assert(
      Array.isArray(realSymArray.vectors) &&
        typeof realSymArray.vectors[0][0] === 'number'
    )

    const genericArray = eigs([
      [0, 1],
      [-1, 0],
    ])
    assert(
      Array.isArray(genericArray.values) &&
        genericArray.values[0] instanceof Complex
    )
    assert(
      Array.isArray(genericArray.vectors) &&
        genericArray.vectors[0][0] instanceof Complex
    )

    const realSymMatrix = eigs(
      matrix([
        [1, 0],
        [0, 1],
      ])
  ...
  it('eigenvalue check for diagonal matrix', function () {
    ...
    approx.deepEqual(
      ...
      [complex(1, 0), complex(2, 1), complex(0, 5)]
    )
  })
  ...
  it('calculates eigenvalues for 2x2 simple matrix', function () {
    ...
    approx.deepEqual(
      ...
      [-0.04795013082563382, 6.047950130825635]
    )
  })
  ...
  it('calculates eigenvalues for 3x3 and 4x4 matrix', function () {
    ...
    approx.deepEqual(
      eigs([
        [
          0.6163396801190624, -3.8571699139231796, 2.852995822026198,
          4.1957619745869845,
        ],
        [
          -3.8571699139231796, 0.7047577966772156, 0.9122549659760404,
          0.9232933211541949,
        ],
        [
          2.852995822026198, 0.9122549659760404, 1.6598316026960402,
          -1.2931270747054358,
        ],
        [
          4.1957619745869845, 0.9232933211541949, -1.2931270747054358,
          -4.665994662426116,
        ],
      ]).values,
      [
        -0.9135495807127523, 2.26552473288741, 5.6502090685149735,
        -8.687249803623432,
      ]
    )
  })
  ...
  it('calculates eigenvalues and eigenvectors for 5x5 matrix', function () {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      for (let j = 0; j < 5; j++) {
        approx.deepEqual(
          divide(eigenRows[i], eigenRows[i][oneIndex[i]]),
          expectedEigenRows[i]
        )
      }
    }
