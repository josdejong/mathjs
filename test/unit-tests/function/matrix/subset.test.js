...
describe('subset', function () {
  ...
  it('should get the right subset of an array', function () {
    ...
    assert.deepStrictEqual(
      subset([math.bignumber(2)], index(0)),
      math.bignumber(2)
    )
  })
  ...
  it('should get the right subset of a matrix', function () {
    assert.deepStrictEqual(
      subset(b, index(new Range(0, 2), 1)),
      matrix([[2], [4]])
    )
  ...
  it('should get a subset of a matrix returning a null or undefined value', function () {
    ...
    assert.deepStrictEqual(subset([null, undefined], index(new Range(0, 2))), [
      null,
      undefined,
    ])
  })
  ...
  it('should set the right subset of an array', function () {
    ...
    assert.deepStrictEqual(subset(d, index(new Range(0, 2), 1), [[-2], [-4]]), [
      [1, -2],
      [3, -4],
    ])
    assert.deepStrictEqual(d, [
    ...
    assert.deepStrictEqual(subset(d, index(2, new Range(0, 2)), [[5, 6]]), [
      [1, 2],
      [3, 4],
      [5, 6],
    ])
    ...
    assert.deepStrictEqual(subset(d, index(0, 0), 123), [
      [123, 2],
      [3, 4],
    ])
  })
  ...
  it('should set the right subset of a matrix', function () {
    ...
    assert.deepStrictEqual(
      subset(g, index(new Range(0, 2), 1), [[-2], [-4]]),
      matrix([
        [1, -2],
        [3, -4],
      ])
    ...
    assert.deepStrictEqual(
      subset(g, index(2, new Range(0, 2)), [[5, 6]]),
      matrix([
        [1, 2],
        [3, 4],
        [5, 6],
  ...
  describe('string', function () {
    ...
    it('should set the right subset of a string', function () {
      ...
      assert.deepStrictEqual(j, 'hello')
      assert.deepStrictEqual(
        subset(j, index(new Range(5, 11)), ' world'),
        'hello world'
      )
      assert.deepStrictEqual(j, 'hello')
    })
  ...
  it('should LaTeX subset', function () {
    ...
    assert.strictEqual(
      expression.toTex(),
      '\\mathrm{subset}\\left(\\begin{bmatrix}1\\end{bmatrix},\\mathrm{index}\\left(0,0\\right)\\right)'
    )
  })
})
