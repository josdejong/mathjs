...
describe('format', function () {
  ...
  describe('non decimal base formatting', function () {
    it('should format in binary, octal, and hexadecimal', function () {
      assert.strictEqual(math.format(128, { base: 2 }), '0b10000000')
      assert.strictEqual(math.format(128, { base: 8 }), '0o200')
      assert.strictEqual(math.format(128, { base: 16 }), '0x80')
      assert.strictEqual(math.format(-128, { base: 16 }), '-0x80')
      assert.strictEqual(math.format(-128, { base: 16, wordSize: 8 }), '0x80')
      assert.strictEqual(math.format(-1, { base: 16, wordSize: 8 }), '0xff')
      assert.strictEqual(math.format(1.25, { base: 2 }), '0b1.01')
      assert.strictEqual(math.format(1.25, { base: 8 }), '0o1.2')
      assert.strictEqual(math.format(1.25, { base: 16 }), '0x1.4')
      assert.strictEqual(math.bin(-128), '-0b10000000')
      assert.strictEqual(math.bin(-128, 8), '0b10000000')
      assert.strictEqual(math.oct(-128), '-0o200')
      assert.strictEqual(math.oct(-128, 8), '0o200')
      assert.strictEqual(math.hex(-128), '-0x80')
      assert.strictEqual(math.hex(-128, 8), '0x80')
    })
    it('should throw an error for invalid values', function () {
      assert.throws(function () { math.format(1.25, { base: 16, wordSize: 8 }) }, 'Error: Value must be an integer')
      assert.throws(function () { math.format(1, { base: 16, wordSize: -8 }) }, 'Error: size must be greater than 0')
      assert.throws(function () { math.format(1, { base: 16, wordSize: 8.5 }) }, 'Error: size must be an integer')
    })
  })
  ...
  describe('bignumber', function () {
    ...
    describe('non decimal base formatting', function () {
      it('should format in binary, octal, and hexadecimal', function () {
        ...
        assert.strictEqual(math.format(math.bignumber(128), { base: 8 }), '0o200')
        assert.strictEqual(math.format(math.bignumber(128), { base: 16 }), '0x80')
        assert.strictEqual(math.format(math.bignumber(-128), { base: 16 }), '-0x80')
        assert.strictEqual(math.format(math.bignumber(-128), { base: 16, wordSize: 8 }), '0x80')
        assert.strictEqual(math.format(math.bignumber(1.25), { base: 2 }), '0b1.01')
        assert.strictEqual(math.format(math.bignumber(1.25), { base: 8 }), '0o1.2')
        assert.strictEqual(math.format(math.bignumber(1.25), { base: 16 }), '0x1.4')
      })
      it('should throw an error for invalid values', function () {
        assert.throws(function () { math.format(math.bignumber(1.25), { base: 16, wordSize: 8 }) }, 'Error: Value must be an integer')
        assert.throws(function () { math.format(math.bignumber(1), { base: 16, wordSize: -8 }) }, 'Error: size must be greater than 0')
        assert.throws(function () { math.format(math.bignumber(1), { base: 16, wordSize: 8.5 }) }, 'Error: size must be an integer')
      })
    })
  })
