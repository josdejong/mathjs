...
describe('round', function () {
  ...
  it('should throw an error on invalid value of n', function () {
    ...
    assert.throws(function () {
      round(1, 1.2)
    }, /TypeError: Number of decimals in function round must be an integer/)
    assert.throws(function () {
      round(1, bignumber(1.2))
    }, /TypeError: Number of decimals in function round must be an integer/)
    assert.throws(function () {
      round(math.complex(1, 1), 1.2)
    }, /TypeError: Number of decimals in function round must be an integer/)
    assert.throws(function () {
      round(math.complex(1, 1), bignumber(1.2))
    }, /TypeError: Number of decimals in function round must be an integer/)
    assert.throws(function () {
  ...
  it('should round bignumbers', function () {
    ...
    assert.deepStrictEqual(
      round(bignumber(2.123456), bignumber(3)),
      bignumber(2.123)
    )
  ...
  it('should round real and imag part of a complex number', function () {
    assert.deepStrictEqual(
      round(math.complex(2.2, math.pi)),
      math.complex(2, 3)
    )
  })
  ...
  it('should round a complex number with a bignumber as number of decimals', function () {
    assert.deepStrictEqual(
      round(math.complex(2.157, math.pi), bignumber(2)),
      math.complex(2.16, 3.14)
    )
  })
  ...
  it('should throw an error if used with a unit', function () {
    assert.throws(
      ...
      TypeError,
      'Function round(unit) not supported'
    )
    assert.throws(
      function () {
  ...
  it('should round each element in a matrix, array, range', function () {
    assert.deepStrictEqual(
      round(math.range(0, 2.1, 1 / 3), 2),
      math.matrix([0, 0.33, 0.67, 1, 1.33, 1.67, 2])
    )
    assert.deepStrictEqual(
      round(math.range(0, 2.1, 1 / 3)),
      math.matrix([0, 0, 1, 1, 1, 2, 2])
    )
