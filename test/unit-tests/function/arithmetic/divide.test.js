...
describe('divide', function () {
  ...
  it('should divide bignumbers', function () {
    ...
    assert.deepStrictEqual(
      divide(bignumber('2.6e5000'), bignumber('2')),
      bignumber('1.3e5000')
    )
  })
  ...
  it('should divide two complex numbers', function () {
    ...
    approx.deepEqual(
      divide(complex(0, 1), complex(0, 0)),
      complex(Infinity, Infinity)
    )
    approx.deepEqual(
      divide(complex(1, 1), complex(0, 0)),
      complex(Infinity, Infinity)
    )
    approx.deepEqual(
      divide(complex(1, -1), complex(0, 0)),
    ...
    approx.deepEqual(
      divide(complex(2, 3), complex(4, 5)),
      complex('0.5609756097560976 + 0.0487804878048781i')
    )
    approx.deepEqual(
      divide(complex(2, 3), complex(4, -5)),
      complex('-0.170731707317073 + 0.536585365853659i')
    )
    approx.deepEqual(
      divide(complex(2, 3), complex(-4, 5)),
      complex('0.170731707317073 - 0.536585365853659i')
  ...
  it('should divide mixed complex numbers and bignumbers', function () {
    ...
    assert.deepStrictEqual(
      divide(bignumber(1), math.complex(2, 4)),
      math.complex(0.1, -0.2)
    )
  })
  ...
  it('should divide a number by a unit', function () {
    ...
    assert.strictEqual(
      divide(10, math.unit(math.fraction(4), 'mg/s')).toString(),
      '5/2 s / mg'
    )
    assert.strictEqual(
      divide(math.fraction(10), math.unit(4, 'mg/s')).toString(),
      '5/2 s / mg'
    )
    assert.strictEqual(
      divide(math.fraction(10), math.unit(math.fraction(4), 'mg/s')).toString(),
      '5/2 s / mg'
    )

    approx.equal(
      math.format(divide(10, math.unit(math.complex(1, 2), 'm/s')), 14),
      '(2 - 4i) s / m'
    )
  })
  ...
  it('should divide two units', function () {
    ...
    assert.strictEqual(
      divide(a, b).to('gal/minute').toString(),
      '1/32 gal / minute'
    )
  ...
  it('should divide one valued unit by a valueless unit and vice-versa', function () {
    ...
    assert.strictEqual(
      divide(math.unit('gal'), math.unit('4 L')).toString(),
      '0.946353'
    )

    assert.strictEqual(
      divide(math.unit('inch'), math.unit(math.fraction(1), 'cm')).toFraction(),
      '127/50'
    )
  })
  ...
  it('should divide units by a big number', function () {
    ...
    assert.strictEqual(
      divide(bignumber(80), math.unit('day')).format({ precision: 50 }),
      '0.92592592592592592592592592592592592592592592592593 mHz'
    )
    assert.strictEqual(
      divide(bignumber(80), math.unit('1 day')).format({ precision: 50 }),
      '0.92592592592592592592592592592592592592592592592593 mHz'
    )
    assert.strictEqual(
      divide(math.unit('day'), bignumber(81)).format({ precision: 50 }),
      '0.012345679012345679012345679012345679012345679012346 day'
    )
    assert.strictEqual(
      divide(math.unit('1 day'), bignumber(81)).format({ precision: 50 }),
      '0.012345679012345679012345679012345679012345679012346 day'
    )

    assert.strictEqual(
      math
        .create({ number: 'BigNumber' })
        .evaluate('round(80 / day * 5 days, 30)')
        .toString(),
      '400'
    )
  })
  ...
  it('should divide each elements in a matrix by a number', function () {
    ...
    assert.deepStrictEqual(divide(a.valueOf(), 2), [
      [0.5, 1],
      [1.5, 2],
    ])
