...
describe('subtract', function () {
  ...
  it('should subtract bignumbers', function () {
    ...
    assert.deepStrictEqual(
      subtract(bignumber('2.3e5001'), bignumber('3e5000')),
      bignumber('2e5001')
    )
    assert.deepStrictEqual(
      subtract(bignumber('1e19'), bignumber('1')),
  ...
  it('should subtract two complex numbers correctly', function () {
    ...
    assert.deepStrictEqual(
      subtract(math.complex(6, 3), math.complex(-2, -2)),
      math.complex('8 + 5i')
    )
    assert.deepStrictEqual(
      subtract(math.complex(3, 4), 10),
      math.complex('-7 + 4i')
    )
    assert.deepStrictEqual(
      subtract(math.complex(3, 4), -2),
      math.complex('5 + 4i')
    )
    assert.deepStrictEqual(
      subtract(math.complex(-3, -4), 10),
  ...
  it('should throw an error for mixed complex numbers and big numbers', function () {
    ...
    assert.deepStrictEqual(
      subtract(math.bignumber(10), math.complex(3, 4)),
      math.complex(7, -4)
    )
  })
  ...
  it('should subtract two fractions', function () {
    ...
    assert.strictEqual(
      subtract(math.fraction(3, 5), math.fraction(1, 5)).toString(),
      '0.4'
    )
    assert.strictEqual(
      subtract(math.fraction(1), math.fraction(1, 3)).toString(),
      '0.(6)'
    )
  })
  ...
  it('should subtract two quantities of the same unit', function () {
    ...
    assert.deepStrictEqual(
      subtract(
        math.unit(math.bignumber(5), 'km'),
        math.unit(math.bignumber(2), 'km')
      ),
      math.unit(math.bignumber(3), 'km')
    )

    assert.deepStrictEqual(
      subtract(
        math.unit(math.complex(10, 10), 'K'),
        math.unit(math.complex(3, 4), 'K')
      ),
      math.unit(math.complex(7, 6), 'K')
    )
    assert.deepStrictEqual(
      subtract(math.unit(math.complex(10, 10), 'K'), math.unit(3, 'K')),
      math.unit(math.complex(7, 10), 'K')
    )
  })
