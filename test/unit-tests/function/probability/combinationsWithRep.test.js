...
describe('combinations', function () {
  ...
  it('should calculate the combinations of n items taken k at a time with BigNumbers', function () {
    ...
    assert.deepStrictEqual(
      combinationsWithRep(math.bignumber(3), math.bignumber(10)),
      math.bignumber(66)
    )
    assert.deepStrictEqual(
      combinationsWithRep(math.bignumber(8), math.bignumber(33)),
      math.bignumber(18643560)
    )
    assert.deepStrictEqual(
      combinationsWithRep(math.bignumber(63), math.bignumber(7)),
      math.bignumber(1078897248)
    )
    assert.deepStrictEqual(
      combinationsWithRep(math.bignumber(25), math.bignumber(6)),
      math.bignumber(593775)
    )
  })
