...
describe('derivative', function () {
  ...
  it('should take the derivative of a OperatorNodes with SymbolNodes', function () {
    ...
    compareString(
      derivativeWithoutSimplify(threeArgMultiplyConstant, 'x'),
      '3 * (1 * sin(x) + x * 1 * cos(x))'
    )
    ...
    const threeArgAddition = new OperatorNode('+', 'add', [
      math.parse('x'),
      math.parse('sin(x)'),
      math.parse('5x'),
    ])
    compareString(
      derivativeWithoutSimplify(threeArgAddition, 'x'),
      '1 + 1 * cos(x) + 5 * 1'
    )

    const threeArgMultiplication = new OperatorNode('*', 'multiply', [
      math.parse('x'),
      math.parse('sin(x)'),
      math.parse('5x'),
    ])
    compareString(
      derivativeWithoutSimplify(threeArgMultiplication, 'x'),
      '1 * sin(x) * 5 x + x * 1 * cos(x) * 5 x + x * sin(x) * 5 * 1'
    )
    ...
    compareString(
      derivativeWithoutSimplify('5x*3x', 'x'),
      '5 * 1 * 3 x + 5 x * 3 * 1'
    )
    ...
    compareString(
      derivativeWithoutSimplify('5 / (3x)', 'x'),
      '-5 * (3 * 1) / (3 x) ^ 2'
    )

    // Quotient rule, d/dx((2x) / (3x + 2)) = ((2*1)(3x + 2) - (2x)(3*1 + 0)) / (3x + 2)^2 = 4 / (3x + 2)^2
    compareString(
      derivativeWithoutSimplify('(2x) / (3x + 2)', 'x'),
      '((2 * 1) * (3 x + 2) - (2 x) * (3 * 1 + 0)) / (3 x + 2) ^ 2'
    )
    ...
    compareString(
      derivativeWithoutSimplify('10^(2x + 2)', 'x'),
      '10 ^ (2 x + 2) * log(10) * (2 * 1 + 0)'
    )
    ...
    compareString(
      derivativeWithoutSimplify('2x^2', 'x'),
      '2 * 2 * 1 * x ^ (2 - 1)'
    )
    ...
    compareString(
      derivativeWithoutSimplify('(x^3 + x)^(5x + 2)', 'x'),
      '(x ^ 3 + x) ^ (5 x + 2) * ((3 * 1 * x ^ (3 - 1) + 1) * (5 x + 2) / (x ^ 3 + x) + (5 * 1 + 0) * log((x ^ 3 + x)))'
    )
  ...
  it('should properly take the derivative of mathematical functions', function () {
    ...
    compareString(
      derivativeWithoutSimplify('sqrt(6x)', 'x'),
      '6 * 1 / (2 * sqrt(6 x))'
    )
    compareString(
      derivativeWithoutSimplify('nthRoot(6x)', 'x'),
      '6 * 1 / (2 * sqrt(6 x))'
    )
    compareString(
      derivativeWithoutSimplify('nthRoot(6x, 3)', 'x'),
      '1 / 3 * 6 * 1 * (6 x) ^ (1 / 3 - 1)'
    )

    compareString(
      derivativeWithoutSimplify('nthRoot((6x), (2x))', 'x'),
      '(6 x) ^ (1 / (2 x)) * ((6 * 1) * 1 / (2 x) / (6 x) + (0 * (2 x) - 1 * (2 * 1)) / (2 x) ^ 2 * log((6 x)))'
    )
    compareString(
      derivativeWithoutSimplify('log((6*x))', 'x'),
      '(6 * 1) / (6 * x)'
    )
    compareString(
      derivativeWithoutSimplify('log10((6x))', 'x'),
      '(6 * 1) / ((6 x) * log(10))'
    )
    compareString(
      derivativeWithoutSimplify('log((6x), 10)', 'x'),
      '(6 * 1) / ((6 x) * log(10))'
    )
    // d/dx(log(2x, 3x)) = ((2 * 1) / (2 * x) * log(3 * x) - log(2 * x) * (3 * 1) / (3 * x)) / log(3 * x) ^ 2 = (log(3x) - log(2x)) / (xlog(3x)^2)
    compareString(
      derivativeWithoutSimplify('log((2x), (3x))', 'x'),
      '((2 * 1) / (2 x) * log((3 x)) - log((2 x)) * (3 * 1) / (3 x)) / log((3 x)) ^ 2'
    )
    ...
    compareString(
      derivativeWithoutSimplify('cos(2x)', 'x'),
      '2 * 1 * -sin(2 x)'
    )
    compareString(
      derivativeWithoutSimplify('tan(2x)', 'x'),
      '2 * 1 * sec(2 x) ^ 2'
    )
    compareString(
      derivativeWithoutSimplify('sec(2x)', 'x'),
      '2 * 1 * sec(2 x) * tan(2 x)'
    )
    compareString(
      derivativeWithoutSimplify('csc(2x)', 'x'),
      '-(2 * 1) * csc(2 x) * cot(2 x)'
    )
    compareString(
      derivativeWithoutSimplify('cot((2x))', 'x'),
      '-(2 * 1) * csc((2 x)) ^ 2'
    )
    compareString(
      derivativeWithoutSimplify('asin((2x))', 'x'),
      '(2 * 1) / sqrt(1 - (2 x) ^ 2)'
    )
    compareString(
      derivativeWithoutSimplify('acos((2x))', 'x'),
      '-(2 * 1) / sqrt(1 - (2 x) ^ 2)'
    )
    compareString(
      derivativeWithoutSimplify('atan((2x))', 'x'),
      '(2 * 1) / ((2 x) ^ 2 + 1)'
    )
    compareString(
      derivativeWithoutSimplify('asec((2x))', 'x'),
      '(2 * 1) / (abs((2 x)) * sqrt((2 x) ^ 2 - 1))'
    )
    compareString(
      derivativeWithoutSimplify('acsc((2x))', 'x'),
      '-(2 * 1) / (abs((2 x)) * sqrt((2 x) ^ 2 - 1))'
    )
    compareString(
      derivativeWithoutSimplify('acot((2x))', 'x'),
      '-(2 * 1) / ((2 x) ^ 2 + 1)'
    )
    compareString(
      derivativeWithoutSimplify('sinh(2x)', 'x'),
      '2 * 1 * cosh(2 x)'
    )
    compareString(
      derivativeWithoutSimplify('cosh(2x)', 'x'),
      '2 * 1 * sinh(2 x)'
    )
    compareString(
      derivativeWithoutSimplify('tanh(2x)', 'x'),
      '2 * 1 * sech(2 x) ^ 2'
    )
    compareString(
      derivativeWithoutSimplify('sech(2x)', 'x'),
      '-(2 * 1) * sech(2 x) * tanh(2 x)'
    )
    compareString(
      derivativeWithoutSimplify('csch(2x)', 'x'),
      '-(2 * 1) * csch(2 x) * coth(2 x)'
    )
    compareString(
      derivativeWithoutSimplify('coth(2x)', 'x'),
      '-(2 * 1) * csch(2 x) ^ 2'
    )
    compareString(
      derivativeWithoutSimplify('asinh((2x))', 'x'),
      '(2 * 1) / sqrt((2 x) ^ 2 + 1)'
    )
    compareString(
      derivativeWithoutSimplify('acosh((2x))', 'x'),
      '(2 * 1) / sqrt((2 x) ^ 2 - 1)'
    )
    compareString(
      derivativeWithoutSimplify('atanh((2x))', 'x'),
      '(2 * 1) / (1 - (2 x) ^ 2)'
    )
    compareString(
      derivativeWithoutSimplify('asech((2x))', 'x'),
      '-(2 * 1) / ((2 x) * sqrt(1 - (2 x) ^ 2))'
    )
    compareString(
      derivativeWithoutSimplify('acsch((2x))', 'x'),
      '-(2 * 1) / (abs((2 x)) * sqrt((2 x) ^ 2 + 1))'
    )
    compareString(
      derivativeWithoutSimplify('acoth((2x))', 'x'),
      '-(2 * 1) / (1 - (2 x) ^ 2)'
    )
    compareString(
      derivativeWithoutSimplify('abs(2x)', 'x'),
      '2 * 1 * abs(2 x) / (2 x)'
    )
    ...
    compareString(
      derivativeWithoutSimplify('pow(x^3 + x, 5x + 2)', 'x'),
      '(x ^ 3 + x) ^ (5 x + 2) * ((3 * 1 * x ^ (3 - 1) + 1) * (5 x + 2) / (x ^ 3 + x) + (5 * 1 + 0) * log(x ^ 3 + x))'
    )
  ...
  it('should take the partial derivative of an expression', function () {
    ...
    compareString(
      derivativeWithoutSimplify('x + log(y)*x', 'x'),
      '1 + log(y) * 1'
    )

    // 2 * 1 * x ^ (2 - 1) + y * 1 + 0 = 2x + y
    compareString(
      derivativeWithoutSimplify('x^2 + x*y + y^2', 'x'),
      '2 * 1 * x ^ (2 - 1) + y * 1 + 0'
    )
  })
  ...
  it('should function properly even without being called within an evaluate', function () {
    ...
    // 2*3*1*x^(3-1) = 6x^2
    compareString(
      derivativeWithoutSimplify(f, 'x').toString(),
      '2 * 3 * 1 * x ^ (3 - 1)'
    )
  })
