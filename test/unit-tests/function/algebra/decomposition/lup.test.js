...
describe('lup', function () {
  it('should decompose matrix, n x n, no permutations, array', function () {
    ...
    const r = math.lup(m)
    // L
    assert.deepStrictEqual(r.L.valueOf(), [
      [1, 0],
      [0.5, 1],
    ])
    // U
    assert.deepStrictEqual(r.U.valueOf(), [
      [2, 1],
      [0, 3.5],
    ])
    // P
    ...
    // verify
    approx.deepEqual(
      math.multiply(_p(r.p), m).valueOf(),
