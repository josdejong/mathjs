...
function assertValidQRDecomposition(A, Q, R) {
  ...
  assert(
    math.equal(
      math.multiply(math.conj(math.transpose(Q)), Q).valueOf(),
      math.identity([Asize[0], Asize[0]]).valueOf()
    ),
  ...
  for (let i = 0; i < rows; i++) {
    for (let j = 0; j < i && j < cols; j++) {
      assert(
        math.isZero(math.subset(R, math.index(i, j))),
        'R is not an upper triangular matrix'
      )
    }
  }
  ...
  for (let i = 0; i < Math.min(rows, cols); i++) {
    ...
    assert(
      !math.isNegative(math.re(diagonalElement)),
      'R has elements on the leading diagonal with a negative real part (R[' +
        i +
        '][' +
        i +
        '] = ' +
...
describe('qr', function () {
  it('should decompose matrix, n x n, no permutations, array', function () {
    ...
    const r = math.qr(m)
    // L
    approx.deepEqual(r.Q.valueOf(), [
      [0.6, -0.8],
      [0.8, 0.6],
    ])
    // U
    approx.deepEqual(r.R.valueOf(), [
      [25, 90],
      [0, 15],
    ])
    // verify
  ...
  it('should decompose matrix, m x n, m > n, dense format', function () {
    ...
    assert.deepStrictEqual(
      ...
      math.matrix([
        [
          0.6531972647421809, -0.0050729188524001045, -0.7248169493126636,
          -0.21897029208715485,
        ],
        [
          0.16329931618554522, -0.13865978196560358, -0.14374377465457616,
          0.9661493287513265,
        ],
        [
          0.7348469228349535, -0.07440280983520192, 0.6732450861047025,
          -0.034717084043718795,
        ],
        [
          0.08164965809277261, 0.9875282032672256, 0.026817368868139818,
          0.13191743558805435,
        ],
      ])
    )
  ...
  it('should decompose matrix, n x n, dense format', function () {
    ...
    assert.deepStrictEqual(
      ...
      math.matrix([
        [
          math.complex(241.44175128417413, 0),
          math.complex(3.3948782289740067, -0.8870876675671249),
        ],
        [
          math.complex(0, 0),
          math.complex(14.254103875042043, -4.440892098500626e-16),
        ],
