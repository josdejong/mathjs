...
describe('complex', function () {
  it('should return 0 + 0i if called with no argument', function () {
    assert.deepStrictEqual(complex(), new math.type.Complex(0, 0))
  ...
  it('should parse a valid string and create the complex number accordingly', function () {
    assert.deepStrictEqual(complex('2+3i'), new math.type.Complex(2, 3))
    assert.deepStrictEqual(complex('2-3i'), new math.type.Complex(2, -3))
    assert.ok(complex('2+3i') instanceof math.type.Complex)
  })

  it('should convert a real number into a complex value', function () {
    assert.deepStrictEqual(complex(123), new math.type.Complex(123, 0))
  })

  it('should convert a fraction into a complex value', function () {
    assert.deepStrictEqual(complex(math.fraction(123)), new math.type.Complex(123, 0))
  })

  it('should convert null into a complex value', function () {
    assert.deepStrictEqual(complex(null), new math.type.Complex(0, 0))
  })

  it('should convert a big number into a complex value (downgrades to number', function () {
    assert.deepStrictEqual(complex(math.bignumber(123)), new math.type.Complex(123, 0))
    assert.deepStrictEqual(complex(math.bignumber(2), math.bignumber(3)), new math.type.Complex(2, 3))
  })

  it('should clone a complex value', function () {
    const b = complex(complex(2, 3))
    assert.deepStrictEqual(b, new math.type.Complex(2, 3))
  })
  ...
  it('should accept polar coordinates as input', function () {
    assert.deepStrictEqual(complex({ r: 1, phi: 1 }), math.type.Complex.fromPolar(1, 1))
    assert.deepStrictEqual(complex({ abs: 1, arg: 1 }), math.type.Complex.fromPolar(1, 1))
  })

  it('should accept an object with im and re as keys', function () {
    assert.deepStrictEqual(complex({ re: 1, im: 2 }), new math.type.Complex(1, 2))
  })
  ...
  it('should create a complex value from a boolean', function () {
    assert.deepStrictEqual(complex(true), new math.type.Complex(1, 0))
  })
  ...
  it('should accept two numbers as arguments', function () {
    assert.deepStrictEqual(complex(2, 3), new math.type.Complex(2, 3))
    assert.deepStrictEqual(complex(2, -3), new math.type.Complex(2, -3))
    assert.deepStrictEqual(complex(-2, 3), new math.type.Complex(-2, 3))
