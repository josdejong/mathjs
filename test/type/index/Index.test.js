...
describe('Index', function () {
  ...
  it('should create an Index', function () {
    assert.deepEqual(new Index(0, 2)._ranges, [new Set([0]), new Set([2])]);

    assert.deepEqual(new Index(new Range(0, 10))._ranges, [{start:0, end:10, step:1}]);
    assert.deepEqual(new Index(new Range(0, 10, 2))._ranges, [{start:0, end:10, step:2}]);
    assert.deepEqual(new Index(new Range(0, 10), new Range(4, 6))._ranges, [
  ...
  it('should create an Index from a Matrix', function () {
    assert.deepEqual(new Index(math.matrix([0, 10]))._ranges, [new Set([0, 10])]);
  });

  it('should create an Index from an array with ranges', function () {
    var index = Index.create([new Range(0, 10), 4]);
    assert(index instanceof Index);
    assert.deepEqual(index._ranges, [new Range(0, 10), new Set([4])]);
  });
  ...
  it('should clone an Index', function () {
    var index1 = new Index(2, new Range(0, 4));
  ...
  it('toJSON', function () {
    assert.deepEqual(new Index(new Range(0, 10), 2).toJSON(),
        {'mathjs': 'Index', ranges: [
          new Range(0, 10, 1),
          new Set([2])
        ]});
  ...
  it('fromJSON', function () {
    ...
    var i1 = new Index(new Range(0, 10), 2);
  ...
  it('should iterate over all ranges', function () {
    var index = new Index(2, new Range(0, 8, 2), new Range(3,-1,-1));
  ...
  it('should expand an index into an array', function () {
    ...
    assert.deepEqual(new Index(new Range(2, 5), new Range(0, 8, 2)).toArray(), [
        [2, 3, 4],
        [0, 2, 4, 6]
    ]);

    assert.deepEqual(new Index(2, new Range(0, 8, 2), new Range(3,-1,-1), new Range(2, 4, 0)).toArray(), [
      [2],
      [0, 2, 4, 6],
      [3, 2, 1, 0],
      []
  ...
  it('valueOf should return the expanded array', function () {
    assert.deepEqual(new Index(2, new Range(0, 8, 2), new Range(3,-1,-1)).valueOf(), [
      [2],
      [0, 2, 4, 6],
      [3, 2, 1, 0]
    ]);
