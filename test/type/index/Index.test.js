...
describe('Index', function () {
  ...
  it('should create an Index from a Range', function () {
    assert.deepEqual(new Index(math.range(0, 10))._ranges, [{start:0, end:10, step:1}]);
  });
  ...
  it('should create an Index from an array with ranges', function () {
    var index = Index.create([math.range(0, 10), 4]);
  ...
  it('should calculate the size of an Index', function () {
    assert.deepEqual(new Index(math.range(0, 10)).size(), [10]);
    assert.deepEqual(new Index(math.range(0, 10, 2)).size(), [5]);
    assert.deepEqual(new Index(math.range(4, 0, -1)).size(), [4]);
    assert.deepEqual(new Index(math.range(0, 10), math.range(4, 6)).size(), [10, 2]);
    assert.deepEqual(new Index(math.range(0, 10), math.range(4, 6), math.range(3, -1, -1)).size(), [10, 2, 4]);
    assert.deepEqual(new Index(math.range(0, 0)).size(), [0]);
    assert.deepEqual(new Index(math.range(0, -1)).size(), [0]);
    assert.deepEqual(new Index().size(), []);
  });

  it('should calculate the minimum values of an Index', function () {
    assert.deepEqual(new Index(math.range(2, 10)).min(), [2]);
    assert.deepEqual(new Index(math.range(4, 0, -1)).min(), [1]);
    assert.deepEqual(new Index(math.range(0, 10), math.range(4, 6)).min(), [0, 4]);
    assert.deepEqual(new Index(math.range(0, 10), math.range(4, 6), math.range(3, -1, -1)).min(), [0, 4, 0]);
    assert.deepEqual(new Index().min(), []);
  });

  it('should calculate the minimum values of an Index', function () {
    assert.deepEqual(new Index(math.range(2, 10)).max(), [9]);
    assert.deepEqual(new Index(math.range(4, 0, -1)).max(), [4]);
    assert.deepEqual(new Index(math.range(0, 10), math.range(4, 6)).max(), [9, 5]);
    assert.deepEqual(new Index(math.range(0, 10), math.range(4, 6), math.range(3, -1, -1)).max(), [9, 5, 3]);
  ...
  it('should test whether an Index contains a scalar', function () {
    ...
    assert.equal(new Index(math.range(2, 10)).isScalar(), false);
    assert.equal(new Index(2, math.range(0, 4), 2).isScalar(), false);
    assert.equal(new Index(math.range(0, 2), math.range(0, 4)).isScalar(), false);
    assert.equal(new Index().isScalar(), true);
  });

  it('should clone an Index', function () {
    var index1 = new Index(2, math.range(0, 4));
  ...
  it('toJSON', function () {
    assert.deepEqual(new Index(math.range(0, 10), 2).toJSON(),
        {'mathjs': 'Index', ranges: [
          math.range(0, 10, 1),
          math.range(2, 3, 1)
        ]});
  ...
  it('should get the range for a given dimension', function () {
    ...
    assert.deepEqual(index.range(0), math.range(2, 3));

    assert(index.range(1) instanceof Range);
    assert.deepEqual(index.range(1), math.range(0, 8, 2));

    assert(index.range(2) instanceof Range);
    assert.deepEqual(index.range(2), math.range(3, -1, -1));
    assert.strictEqual(index.range(3), null);
  });

  it('should iterate over all ranges', function () {
    var index = new Index(2, math.range(0, 8, 2), math.range(3,-1,-1));
    ...
    assert.deepEqual(log, [
      {range: math.set([2]), index: 0},
      {range: math.range(0, 8, 2), index: 1},
      {range: math.range(3, -1, -1), index: 2}
    ]);
  });
  ...
  it('should expand an index into an array', function () {
    assert.deepEqual(new Index(math.range(2, 5)).toArray(), [
      [2, 3, 4]
    ]);

    assert.deepEqual(new Index(math.range(2, 5), math.range(0, 8, 2)).toArray(), [
        [2, 3, 4],
        [0, 2, 4, 6]
    ]);

    assert.deepEqual(new Index(2, math.range(0, 8, 2), math.range(3,-1,-1), math.range(2, 4, 0)).toArray(), [
      [2],
      [0, 2, 4, 6],
      [3, 2, 1, 0],
      []
  ...
  it('valueOf should return the expanded array', function () {
    assert.deepEqual(new Index(2, math.range(0, 8, 2), math.range(3,-1,-1)).valueOf(), [
      [2],
      [0, 2, 4, 6],
      [3, 2, 1, 0]
    ]);
