...
describe('cube', function() {
  ...
  it('should cube Quaternions',function() {
    assert.deepEqual(cube(math.quaternion({i:1})), math.quaternion({i:-1}));
    assert.deepEqual(cube(math.quaternion({j:1})), math.quaternion({j:-1}));
    assert.deepEqual(cube(math.quaternion({r:1})), math.quaternion({r:1}));
    assert.deepEqual(cube(math.quaternion({k:1})), math.quaternion({k:-1}));
    assert.deepEqual(cube(math.quaternion(1,2,3,4)), math.quaternion(-86,-52,-78,-104));
    assert.deepEqual(cube(math.quaternion(3,-2,-3,4)), math.quaternion(-234,4,6,-8))
  });
