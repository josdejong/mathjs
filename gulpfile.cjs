...
const IMPORT_COREJS = `import 'core-js/stable'
`
...
// generate defaultInstance with core-js imports
function generateCorejsEntry (done) {
  const defaultInstanceSrc = fs.readFileSync(BUNDLE_ENTRY)

  fs.writeFileSync(BUNDLE_ENTRY_COREJS, `${IMPORT_COREJS}${defaultInstanceSrc}${AUTOGENERATED_WARNING}`)
