...
obj.varName = "";
obj.numVar = 0;
var moreThanOne = obj.moreThanOne 
    ? true 
    : false;

try {
   recPoly(node,obj) 
   if (obj.numVar===0)  return 'Expression does not contains any variable.'
   return true;
} catch (e) {
  return e
}
...
var sBefore;      // previou expression
...
while (true) {
   ...
   node = m.simplify(node,(firstLoop
                           ? setRules.firstRules 
                           : setRules.sucDivRules));       
...
var arrR, arrC, solutions;
  ...
  // Coefficients with increasing exponents (for Durand-Kerner)
var incrExpo = oSimpl.coef        

     // Clone and reverte for Jenkins Traub method. 
var decrExpo = incrExpo.slice().reverse();  
...
if (nRoots===0)
   l('There is no real root in equation')
    ...
    for (var i=0;i<realRoots.length ;i++) {
      scope[obj.varName] = realRoots[i]     
...
// Calculates the lesser exponent, that in polynominal with all coefficients is 0.
var nRed = 0;   
if (reduce) {
  for (i=0;i<maxExpo;i++)  {
    if (coef[i]!==0) break
...
//         x 2    5 x
   ...
   var n1  = new m.expression.node.ConstantNode(
                  first 
                      ? coef[i] 
                      : Math.abs(coef[i]));
   var op = coef[i]<0 
   ...
   if (i>0)   {  // Is not a constant without variable 
     var n2 = new m.expression.node.SymbolNode(varname);    
     if (i>1)    {
       var n3 =  new m.expression.node.ConstantNode(i);     
