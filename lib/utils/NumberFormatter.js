...
NumberFormatter.prototype.toEngineering = function (precision) {
  ...
  var value = Math.abs(parseFloat(rounded.toExponential()));
  ...
  var newExp = e < 0 ? (e - 3) - (e % 3) : e - (e % 3);

  // find difference in exponents
  var expDiff = Math.abs(e - newExp);

  // cut off leading 0's
  while (valueStr[0] === '0') valueStr = valueStr.substr(1);

  while (--expDiff >= 0) {
    console.log('exp diff: ' + expDiff);
    console.log('decimal idx: ' + decimalIdx);
    ++decimalIdx;
  }

  valueStr = valueStr.replace(/\./, '');
  valueStr = valueStr.slice(0, decimalIdx) + '.' + valueStr.slice(decimalIdx);

  // if everything to the right is zeros
  if (parseInt(valueStr.substr(decimalIdx + 1)) === 0) {
    valueStr = valueStr.substr(0, decimalIdx);
  }

  if (decimalIdx === valueStr.length - 1) {
    valueStr = valueStr.replace(/\./, '');
  }

  valueStr += 'e' + (e >= 0 ? '+' : '') + newExp;
  return s + valueStr;
}

/*NumberFormatter.prototype.toEngineering = function (precision) {
  var rounded = precision ? this.roundDigits(precision) : this.clone();
  var value = rounded.toFixed();
  var valueCoefficients = value.toString().split('');
  console.log('value coeffs: ' + valueCoefficients);
  if (valueCoefficients.length === 1) return this.toExponential();
  ...
  // find nearest lower multiple of 3 for exponent
  var newExp = s === '-' ? (e - 3) - (e % 3) : e - (e % 3);
  console.log('new exponent: ' + newExp);

  // find difference in exponents
  var expDiff = Math.abs(e - newExp);
  console.log('exp difference ' + expDiff);

  var significands = [];

  while (expDiff-- >= 0) {
    significands.push(valueCoefficients.shift());
  }
  console.log('significands: ' + significands);

  return this.sign + significands.join('') + 'e' + (newExp >= 0 ? '+' : '') + newExp;
};*/
