...
module.exports = function asinAcsc(x, BigNumber, reciprocal) {
  ...
  if (reciprocal) {
    ...
    absX = x.abs();
  } else if (absX.gt(new BigNumber(1))) {
  ...
  if (absX.gt(0.8)) {
    ...
    var sign = x.s;
    var halfPi = pi(BigNumber.clone({precision: precision + 4})).div(2);
    x = halfPi.minus(asinAcsc(new BigNumber(1).minus(x.times(x)).sqrt(), BigNumber));
    x.s = sign;
  ...
  if (wasReduced) {
    ...
    x = x.div(new BigNumber(2).sqrt().times(one.minus(x.times(x)).sqrt()
        .plus(one).sqrt()));
