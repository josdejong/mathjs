...
module.exports = function atanAcot (x, BigNumber, reciprocal) {
  ...
  if ((!reciprocal && !x.isFinite()) || (reciprocal && x.isZero())) {
    var halfPi = constants.pi(BigNumber.clone({precision: precision + 2})).div(2).toDP(precision - 1);
  ...
  if (absX.gte(1.143)) {
    ...
    var halfPi = constants.pi(BigNumber.clone({precision: precision + 4})).div(2);
    var ret = halfPi.minus(atan(new BigNumber(1).div(absX)));
    ret.s = x.s;
