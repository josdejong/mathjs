...
function _validate(array, size, dim) {
  ...
  if (len != size[dim]) {
    throw new error.DimensionError(len, size[dim]);
  }
  ...
  if (dim < size.length - 1) {
    ...
    for (i = 0; i < len; i++) {
      var child = array[i];
      if (!isArray(child)) {
        throw new error.DimensionError(size.length - 1, size.length, '<');
      }
...
exports.validate = function validate(array, size) {
  ...
  if (isScalar) {
    // scalar
    if (isArray(array)) {
      throw new error.DimensionError(array.length, 0);
    }
  }
  else {
    // array
...
exports.validateIndex = function validateIndex (index, length) {
  ...
  if (length !== undefined && index >= length) {
    throw new error.IndexError(index, length);
  }
};
