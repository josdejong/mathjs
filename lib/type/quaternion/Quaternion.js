...
function factory (type, config, load, typed, math) {
   ...
   */
    ...
    if (this.j === 0 && this.k === 0) {
      this.canConvertToComplex = true;
    } else {
      this.canConvertToComplex = false;
    }
	}
    ...
    if (a instanceof Quaternion) {
      ...
      typeof c === 'number' || typeof d === 'number' ) && typeof c !== 'object'
    ) { // type of c must not be object to stop mod arg unit vector quaternions being caught here
      Q.r = parseFloat(a) || 0;
      Q.i = parseFloat(b) || 0;
      Q.j = parseFloat(c) || 0;
      Q.k = parseFloat(d) || 0;
      ...
      && typeof a.phi === 'undefined'
      // to prevent the r from modulus argument form complex number being used
      // as a scaler Quaternion r must be undefined
    ) {
      Q.r = parseFloat(a.r) || 0;
      Q.i = parseFloat(a.i) || 0;
      Q.j = parseFloat(a.j) || 0;
      Q.k = parseFloat(a.k) || 0;
    } else if (typeof a === 'object' && a !== null && ('re' in a || 'im' in a) && b === undefined) {
      Q.r = parseFloat(a.re) || 0; // b must be undefined so it is not mistaken for a pair of complex numbers
      ...
      /* The regex will match any number of any of the following examples:
       *
       ...
       * This is why a '+' is appended to the start of the string (even if it already has a + or -)
      */
      var results = a.match(identifyTerm);
      var identifyInvalid = /[ijk][ijk]/; // check for consecutive i, j or ks
      if (a.search(identifyInvalid) !== -1) {
        makeQuartError();
      }
      if (results.length <1) {
        makeQuartError();
      ...
      for (var i = 0; i<results.length; i++) {
        ...
        var component = (last_char === 'i' || last_char === 'j' || last_char === 'k')
          ? last_char
          : 'r';
        var value = 0;
        if(component === 'r'){
    ...
    } else if (// support for quaternions in form (3 + 4i) + (1 - 2i)j (passed in as 2 complex numbers)
...
/*
 * for a complex 2x2 matrix to be converted to a quaternion (a + bi + cj + dk) it must take the form:
 *
 *      (a + bi) , (c + di)       A , B
 * M =                        =
 *      (-c + di) , (a - bi)      C , D
 *
 * easiest check for this is that
 *
 * A === D* && -B === C*
 *
 * should be true
*/
      if (!(array[0][0] !== array[1][1].conjugate() ) || !(array[0][1].mul(-1,1) !== array[1][0].conjugate())) {
        makeQuartError();
      }
 ...
 */
      for (var i = 0; i < 3; i++) {
        for (var j = i + 1; j < 3; j++) {// j = i + 1 prevents elements being check twice
      ...
      if( array[0][0] === array[1][1] &&
        (array[0][0] === array[2][2]) && (array[0][0] === array[3][3])) {
    ...
    } else if (typeof a === 'object' && 'mod' in a && 'arg' in a && 'unit' in a && // check that the write components are in the object
       ('i' in a.unit && 'j' in a.unit && 'k' in a.unit && ( !('r' in a.unit) || a.unit.r === 0))) { // check that unit vector is valid
      /*
       ...
       */
       a.unit = new Quaternion(0,a.unit.i, a.unit.j, a.unit.k).normalise();
       var sinArgMod = Math.sin(a.arg)* a.mod;
       Q.r = a.mod * Math.cos(makePrincipleArgument(a.arg));
       Q.i = a.unit.i * sinArgMod;
       Q.j = a.unit.j * sinArgMod;
       Q.k = a.unit.k * sinArgMod;
  ...
  function checkFor4x4Matrix(a) {
    ...
    for (var i = 0; i < 4 ;i++) {
      if (!(Array.isArray(array[i])) || array[i].length !== 4) {
        return false;
      } else {
        for (var j = 0; j < 4; j++) {
  ...
  function checkFor2x2Matrix(a) {
    ...
    for (var i = 0; i < 2 ;i++) {
      if(!(Array.isArray(array[i])) || array[i].length !== 2) {
        ...
        for( var j = 0; j < 2; j++) {
          if ( !('re' in array[i][j]) || !('im' in array[i][j])) {
            return false;
          }
        }
      }
  ...
  */
  ...
  Quaternion.prototype.reverseOrderSubtract = function(q) {
    parseQuaternion(q);
    return (new Quaternion(Q.r, Q.i, Q.j, Q.k)).subtract(this);
  };
  ...
  /*
   * finds the multiplicative inverse of a Quaternion by
   * dividing the conjugate by the norm
   *
   * @returns {Quaternion}
   */
  ...
  /*
   * finds the additive inverse by multiplying by -1
   *
   * @returns {Quaternion}
   */
  ...
  Quaternion.prototype.ceil = function(places) {
    if (typeof places === 'undefined' || places !== Math.abs(places)) {
      places = 1;
    } else if (places !== parseInt(places)) {
      places = Math.pow(10,Math.floor(places));
    } else {
  ...
  /*
   * multiples q by this (the reverse order to usual)
   *
   * this method is used when wanting to compute (3+ 4i)(5 - 2i +3j - 5k)
   * as the quaternion multiplication method cannot be called form the complex number
   *
   * @returns {Quaternion}
   * @nonComut
   */
  ...
  /*
   * divides this by a Quaternion by multiplying by its multiplicative inverse
   *
   * @returns {Quaternion}
   * @nonComut
   */
   ...
   * can be more accurate than finding the multiplicative inverse of a scalar then multiplying
   *
   * @returns {Quaternion}
   */
  ...
  Quaternion.prototype.dotDivide = function(q) {
    ...
    if(Q.r === 0 || Q.i === 0 || Q.j === 0 || Q.k === 0) {
      throw TypeError('Cannot divide by zero');
    }
    return new Quaternion(this.r / Q.r, this.i / Q.i, this.j / Q.j, this.k / Q.k);
  }
  ...
  Quaternion.prototype.dotPow = function(e){ // performs the power on each individual element
    if (e !== parseInt(e) && (this.r < 0 || this.i < 0 || this.j < 0 || this.k < 0) ) {
      throw TypeError('Cannot raise negative number to non intiger power');
    } else {
      return new Quaternion(
  ...
  Quaternion.prototype.exp = function() {
    ...
    * where v is a unit vector containing the i, j and k components
    ...
    */
    ...
    return (vOnVNorm.mul(new Quaternion({r:sinOfVNorm}))
      .add(new Quaternion({r:cosVnorm})))
        .mul(new Quaternion({r:eToTheA}));
  }
   ...
   * @returns {Quaternion}
   */
  Quaternion.prototype.ln = function () {
     /*
     * this is computed using the formula form https://en.wikipedia.org/wiki/Quaternion
     *
     * the formula is:
    ...
    var vOnVnorm = new Quaternion(0, this.i/vNorm, this.j/vNorm, this.k/vNorm);
    return vOnVnorm.mul({r:aCosOfAOnQNomr})
      .add(new Quaternion(Math.log(qNomr),0,0,0));
  }
  ...
  Quaternion.prototype.pow = function(p) {
    ...
    if (this.canConvertToComplex && p.canConvertToComplex) {
    ...
    if (p.isScalar) {
      var oldModArgUnit = this.toModArgUnitForm()
      return new Quaternion({
        mod: Math.pow(oldModArgUnit.mod, p),
        arg: makePrincipleArgument(oldModArgUnit.arg * p.r),
        unit: oldModArgUnit.unit
      });
    }
  ...
  function makePrincipleArgument (arg) {
  ...
  // can be used instead of pow but can also be used to find the square root of
  // negative real numbers when constraints have been passed in
  Quaternion.prototype.nthRoot = function(root, constraint,unknownComponent) {
    if(!this.isPure && this.r <0 && root === 2) {
      constraint = new Quaternion(constraint);
      ...
      if(constraint[unknownComponent] !== 0) {
        throw TypeError('Unknown component can not be constraint');
      }

      var P = constraint;

      var constraintSizeTest = -this.r-P.i*P.i-P.j*P.j-P.k*P.k;

      if(constraintSizeTest<0) {
  ...
  /*
   * performs power in the reverse order (uses this as the exponent and p as the base)
   *
   * @returns {Quaternion}
   * @nonComut
   */
  ...
  Quaternion.prototype.arg = function() {
    if(this.isScalar && this.isPure){
      return 0;
    } else {
      return Math.acos(this.r/this.norm());
    }
  }

  /*
   * gets a unit vector for mod, arg, unit form
   * by ignoring the real component and normalising the rest
   *
   * @returns {number}
   */

  Quaternion.prototype.getUnitVector = function() {
    var q = new Quaternion(0,this.i, this.j, this.k);
    return q.normalise();
  }

  Quaternion.prototype.toModArgUnitForm = function() {
    return {
      arg: this.arg(),
      mod: this.norm(),
      unit: this.getUnitVector()
    }
  }
  ...
  /*
   * find the norm of the quaternion and divides each element by it
   * this means that the norm of the new quaternion is 1
   *
   * @returns {Quaternion}
   */
   ...
   Quaternion.prototype.to4x4Matrix = function() {
    return [
      ...
      [this.k, -this.j, this.i, this.r]
        ...
        //util function
  /*
   * this method is used when the starting perimeter is not a Quaternion
   * this is needed because many Quaternion operators are not comutative
   * example:
   * used when wanting to compute (3+2i) * (3 + 7i + 3j + 5k)
   * because (3+2i) is not a Quaternion the multiply method
   * cannot be called form it so it is passed into nonQuaternionOp() with the name of the function
   * (3+2i) * (3 + 7i + 3j + 5k) = A
   *
   * A = nonQuaternion([new Complex(f3+2i), new Quaternion(3 + 7i + 3j + 5k)], 'mul');
   *
   * this is necessary because Quaternions are not comutative
