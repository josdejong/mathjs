...
function factory (type, config, load, typed, math) {
  ...
  */
    ...
    if (!this.isPure && this.j === 0 && this.k === 0 || this.isPure && this.isScalar) {
  ...
  */
   ...
   */
   ...
   /*
    * All Quaternion polynomial has infinite roots meaning that
    *
    * var a = new Quaternion(1,2,3,4)
    * a.pow({r:3}).pow({r:1/3}) != a
    *
    * but
    *
    * a.pow({r:3}).pow({r:1/3}.pow({r:3})) == a.pow({r:3})
    */
  Quaternion.prototype.pow = function(p) {
    ...
    if (this.r === 1 && this.i === 0 && this.j === 0 && this.k === 0) {
      return this;
    }
  ...
  function buildToString(string, value, component){
    ...
    if(string === ''){
      ...
      : ' - ' + Math.abs(value.toString()) + component;
    }
  }
  ...
  //  basic trig function
  Quaternion.prototype.sin = function() {
    if (this.canConverToComlex) {
      var c = new Complex(this.r, this.i).sin();
      return new Quaternion(c.re, c.im, 0, 0).round(15);
    }
    /*
     * calculated using the formula:
     * Sin(q) = [e^(i*q)-e^(-i*q)]/2i
     */
    var iTimesQ = (new Quaternion({i:1})).mul(this);
    var minusITimesQ = (new Quaternion({i:-1})).mul(this);
    return new Quaternion((iTimesQ.exp().subtract(minusITimesQ.exp())).divide({i:2})).round(15);
  }

  Quaternion.prototype.cos = function(){
    if (this.canConverToComlex) {
      var c = new Complex(this.r, this.i).cos();
      return new Quaternion(c.re, c.im, 0, 0).round(15);
    }
    /*
     * calculated using the formula:
    ...
    var iTimesQ = (new Quaternion({i:1})).mul(this);
    var minusITimesQ = (new Quaternion({i:-1})).mul(this);
    return new Quaternion( iTimesQ.exp().add(minusITimesQ.exp() ).divide(2)).round(15);
  }

  Quaternion.prototype.tan = function() {
    if (this.canConverToComlex) {
      var c = new Complex(this.r, this.i).tan();
      return new Quaternion(c.re, c.im, 0, 0).round(15);
    }
    // using identity tan(x) = sin(x)/cos(x)
    return this.sin().divide(this.cos()).round(15);
  }

  // recipricals of basic trig funcitons
  Quaternion.prototype.cosec = function() {
    if (this.canConverToComlex) {
      var c = new Complex(this.r, this.i).csc();
      return new Quaternion(c.re, c.im, 0, 0).round(15);
    }
    return this.sin().reciprocal().round(15);
  }

  Quaternion.prototype.sec = function() {
    if (this.canConverToComlex) {
      var c = new Complex(this.r, this.i).sec();
      return new Quaternion(c.re, c.im, 0, 0).round(15);
    }
    return this.cos().reciprocal().round(15);
  }

  Quaternion.prototype.cot = function() {
    if (this.canConverToComlex) {
      var c = new Complex(this.r, this.i).cot();
      return new Quaternion(c.re, c.im, 0, 0).round(15);
    }
    return this.cos().divide(this.sin()).round(15);
  }
  ...
  Quaternion.prototype.asin = function() {
    if (this.canConverToComlex) {
      var c = new Complex(this.r, this.i).asin();
      return new Quaternion(c.re, c.im, 0, 0).round(15);
    }
    /*
     ...
     */
     ...
     var Iq = I.mul(this);
     var rootVal = (new Quaternion({r:1})).subtract(this.square()).pow(0.5);
     var lnVal = Iq.add(rootVal).ln();
     return minusI.mul(lnVal).round(15);
  }

  Quaternion.prototype.acos = function() {
    // if (this.canConverToComlex) {
    //   var c = new Complex(this.r, this.i).acos();
    //   return new Quaternion(c.re, c.im, 0, 0).round(15);
    // }
    return (new Quaternion(Math.PI/2,0,0,0)).subtract(this.asin()).round(15);
  }

  Quaternion.prototype.atan = function() {
    if (this.canConverToComlex) {
      var c = new Complex(this.r, this.i).atan();
      return new Quaternion(c.re, c.im, 0, 0).round(15);
    }
    /*
     * calculated using formula:
     *
     * atan(q) = (i/2)*[ ln(1 - i*q) - ln(1 + i*q) ] === (i/2)*ln[ (1 - i*q)/(1 + i*q)]
     * the second formula is used because calculating fewer logs reduces time
     */
     var one = new Quaternion({r:1});
     var i = new Quaternion({i:1});
     var iOn2 = new Quaternion({i:1/2});
     var oneMinusIq = one.subtract(i.mul(this));
     var oneAddIq = one.add(i.mul(this));
     return (new Quaternion(0,0.5,0,0)).mul((oneMinusIq.divide(oneAddIq).ln())).round(15);
  }
  ...
  Quaternion.prototype.sinh = function() {
    if (this.canConverToComlex) {
      var c = new Complex(this.r, this.i).sinh();
      return new Quaternion(c.re, c.im, 0, 0).round(15);
    }
    /*
     ...
     * sinh(q) = (e^q - e^-q)/2
     */
    return this.exp().subtract(this.negative().exp()).mul({r:0.5}).round(15);
  }

  Quaternion.prototype.cosh = function() {
    if (this.canConverToComlex) {
      var c = new Complex(this.r, this.i).cosh();
      return new Quaternion(c.re, c.im, 0, 0).round(15);
    }
    /*
     * calculating using formula:
     ...
     * cosh(q) = (e^q + e^-q)/2
     */
     return this.exp().add(this.negative().exp()).mul({r:0.5}).round(15);
  }

  Quaternion.prototype.tanh = function() {
    if (this.canConverToComlex) {
      var c = new Complex(this.r, this.i).tanh();
      return new Quaternion(c.re, c.im, 0, 0).round(15);
    }
    /*
     ...
     */
    return this.sinh().divide(this.cosh()).round(15);
  }
