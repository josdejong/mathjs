...
function factory (type, config, load, typed, math) {
  ...
  */
    ...
    if (Q.r === 0 && Q.i !== 0 && Q.j !== 0 && Q.k !== 0){
      this.isPure = true;
      this.isScalar = false;
    } else if (Q.i === Q.j === Q.k ===0){ // 0 + 0i + 0j + 0k is still a scaler Quarternion
      this.isPure = false;
      this.isScalar = true;
    } else {
      this.isPure = false;
  ...
  Quarternion.prototype.type = 'Quarternion';
  ...
  */
  ...
  Quarternion.prototype.reverseOrderSubtract = function(q) {
    makeQuarternion(q);
    return (new Quarternion(Q)).subtract(this);
  };
  ...
  Quarternion.prototype.dotmul = function(q) {
    makeQuarternion(q);
    return new Quarternion(this.r * Q.r, this.i * Q.i, this.j * Q.j, this.k * Q.k);
  }
  ...
  Quarternion.prototype.reverseOrderMultiply = function(q) {
    makeQuarternion(q);
    return (new Quarternion(Q)).mul(this);
  }

  Quarternion.prototype.divide = function(q) {
    makeQuarternion(q)
    Q = (new Quarternion(Q)).reciprocal();
    return this.mul(Q);
  }
  ...
  Quarternion.prototype.ln = function() {// possibly impliment passing in peramiters later
     ...
     */  
    var qNomr = this.norm();
    var vNorm = new Quarternion({r:Math.pow(this.i*this.i, this.j*this.j, this.k*this.k,0.5)});
    var aCosOfAOnQNomr = Complex.acos(this.r/qNomr);// uses Complex acos as the value may be out side of the real acos range
    return (new Quarternion(0,this.i,this.j,this.k)).divide(vNorm).mul(aCosOfAOnQNomr).add(Math.log(qNomr),0,0,0); 
  }
  ...
  Quarternion.prototype.reverseOrderPower = function(p) {
    makeQuarternion(p);
    return (new Quarternion(P)).pow(this);
  }
  ...
  Quarternion.prototype.logn = function(base) {
    /*
     * this is done by using the formula to change the base of a number
     * log_n(q) = log_e(q)/log_e(n)
     *
     */
    makeQuarternion(base);
    n = new Quarternion(Q.r, Q.i, Q.j, Q.k);
  ...
  Quarternion.prototype.asin = function() {
     ...
     */ 
     var minusI = new Quarternion({i:-1});
     var I = new Quarternion({i:1});
     var rootVal = (new Quarternion(this.RE)).subtract(this.square()).pow(0.5);
  ...
  Quarternion.prototype.atanh = function() {
    /*
     * calculating using formula: 
     *
