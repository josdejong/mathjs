...
/**
 * Get string representation, optionally rounding to a specified
 * number of digits.
 * @param {Number} [precision]   Number of digits to round the numbers
 * @return {String}
 */
Unit.prototype.format = function format(precision) {
  var value,
      str;
  ...
  if (!this.fixPrefix) {
    ...
    str = (this.value != null) ? number.format(value, precision) + ' ' : '';
