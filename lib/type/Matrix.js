...
module.exports = function (config) {
  ...
  function Matrix(data, format) {
    ...
    if (data instanceof Matrix) {
      ...
      // check we need to change format
      /*if (!format || format === data._storage.format()) {
        // clone storage
        this._storage = data._storage.clone();
      }
      else {
        // change format, TODO: find a better solution
        this._storage = createStorage(data._storage.toArray(), format);
      }*/
    }
      ...
      // create storage
      //this._storage = createStorage(data, format);
    }
    else if (data != null) {
      // unsupported type
      ...
      this._data = [];
      // create storage
      //this._storage = createStorage([], format);
    }
  ...
  var createStorage = function (data, format) {
    // format to use
    ...
    if (!f)
      throw new SyntaxError('Unsupported Matrix Storage Format: ' + f);
    // create instance
    return new f(data);
  };
  ...
  Matrix.prototype.clone = function () {
    ...
    //matrix._storage = this._storage.clone();
    return matrix;
  };
