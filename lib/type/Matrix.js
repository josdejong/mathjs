...
/**
 * Get a single value from an array. The function tests whether:
...
/**
 * Replace a single value in an array. The function tests whether index is a
...
function _setScalar1D (data, size, index, value) {
  ...
  if (row + 1 > size[0]) {
    array.resize(data, [row + 1]); // size is index + 1 as index is zero-based
    size[0] = row + 1;
  }
  data[row] = value;
}
...
Matrix.prototype.resize = function resize(size, defaultValue) {
  ...
  array.resize(this._data, this._size, defaultValue);
