...
/**
 * Format the complex number as a string, optionally rounding to a specified
 * number of digits.
 * @param {Number} [precision]   Number of digits to round the numbers
 * @return {String} str
 */
Complex.prototype.format = function format (precision) {
  var str = '',
      strRe = number.format(this.re, precision),
      strIm = number.format(this.im, precision);
  ...
  if (this.im == 0) {
    ...
    if (this.im > 0) {
      ...
      if (this.im == -1) {
        ...
        str = strRe + ' - ' +
            number.format(Math.abs(this.im), precision) + 'i';
      }
    }
  }
