...
/**
 * Get the nth root of a complex number in polar form (De Moviers Theorem)
 * @param  {Number} n the nth root to be found
 * @return {Array}  an array with n polar form objects representing the roots of the Complex number
 */
Complex.prototype.nthRoot = function(n) {
  var polar = this.toPolar();
  var roots = [];
  var r = Math.pow(polar.r, 1/n);

  for(var k = 0; k < n; k++) {
    roots.push({r: r, phi: (polar.phi + 2 * Math.PI * k)/n});
  }
