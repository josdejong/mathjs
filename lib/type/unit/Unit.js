...
function factory (type, config, load, typed) {
  ...
  Unit.prototype.format = function (options) {
    ...
    // Units must have only one unit and not have the fixPrefix flag set and not be Complex
    if (this.units.length === 1 && !this.fixPrefix && !(this.value && this.value.isComplex)) {
    ...
    if(this.value && this.value.isComplex) {
      str = "(" + str + ")";    // Surround complex values with ( )
    }
  ...
  /**
   * Create a value with the same type as provided exampleType
   * @param {number | BigNumber | Fraction} value
   * @param {number | BigNumber | Fraction} exampleValue
   */
  function convertToTypeOf(value, exampleValue) {
    if (exampleValue) {
      if (exampleValue.isBigNumber) {
        return new type.BigNumber(value);
      }
      if (exampleValue.isFraction) {
