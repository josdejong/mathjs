...
function factory (type, config, load, typed) {
  ...
  Unit.prototype._normalize = function (value) {
    if (this.units.length === 0) {
      ...
      var res = value;
      for(var i=0; i < this.units.length; i++) {
        res = multiply(res, pow(multiply(this.units[i].unit.value, this.units[i].prefix.value), this.units[i].power));
      ...
      return multiply(add(value, this.units[0].unit.offset), multiply(this.units[0].unit.value, this.units[0].prefix.value));
    }
  };
  ...
  Unit.prototype._denormalize = function (value, prefixValue) {
    if (this.units.length === 0) {
      ...
      var res = value;
      for(var i=0; i<this.units.length; i++) {
        res = divide(res, pow(multiply(this.units[i].unit.value, this.units[i].prefix.value), this.units[i].power));
      }
      return res;
    }
    else {
      ...
      if (prefixValue == undefined) {
        return subtract(divide(divide(value, this.units[0].unit.value), this.units[0].prefix.value), this.units[0].unit.offset);
      }
      else {
        return subtract(divide(divide(value, this.units[0].unit.value), prefixValue), this.units[0].unit.offset);
      }
    }
  };
