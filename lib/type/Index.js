...
Index.prototype.toString = function () {
  var strings = [],
      precision = options.format.precision;

  for (var i = 0, ii = this._ranges.length; i < ii; i++) {
    var range = this._ranges[i];
    var str = number.format(range.start, precision);
    if (range.step != 1) {
      str += ':' + number.format(range.step, precision);
    }
    str += ':' + number.format(range.end, precision);
    strings.push(str);
  }
