...
function factory (type, config, load, typed) {
  ...
  var _getsubset = function (matrix, idx) {
    ...
    var msize = matrix._size;
    
    // rows in matrix
    var r = msize[0];
    
    // rows & columns for result matrix
    var rows = idx.dimension(0).toArray();
    var columns = idx.dimension(1).toArray();
    ...
    // dump rows to process into workspace
    ...
    for (p = 0, pp = columns.length; p < pp; p++) {
      // update ptr
