...
module.exports = function (math) {
  ...
  CrsMatrix.prototype.get = function (index, d) {
    if (!isArray(index))
  ...
  CrsMatrix.prototype.set = function (index, v) {
    if (!isArray(index))
      throw new TypeError('Array expected');
    if (index.length != this._size.length)
      throw new DimensionError(index.length, this._size.length);
  ...
  CrsMatrix.prototype.format = function (options) {
    ...
    var columns = this._size[1];
    // rows & columns
    var str = 'CRS [' + string.format(rows) + ' x ' + string.format(columns) + ']\n';
    // values index
    var k = 0;
    // loop rows
    for (var i = 0; i < rows; i++) {
  ...
  CrsMatrix.prototype.multiply = function (x) {
  ...
  CrsMatrix.prototype.transpose = function () {
    ...
    // crs transpose is a crs matrix with the same structure
  ...
  CrsMatrix.diagonal = function (rows, columns, value) {
    // create arrays
    var values = [];
    var index = [];
    var ptr = [];
