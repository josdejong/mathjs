...
function factory (type, config, load, typed) {
  ...
  var _createFromMatrix = function (matrix, source) {
    ...
    if (source.type === 'CrsMatrix') {
    ...
    else if (source.type === 'CcsMatrix') {
      // ccs arrays
      var svalues = source._values;
      var sindex = source._index;
      var sptr = source._ptr;
      var ssize = source._size;
      // rows and columns
      var rows = ssize[0];
      var columns = ssize[1];
      // non-zero elements
      var nzl = svalues.length;
      // ccs arrays
      var values = new Array(nzl);
      var index = new Array(nzl);
      var ptr = new Array(rows + 1);
      // vars
      var i, j, k, n;
      // initialize ptrs
      for (i = 0; i <= rows; i++) 
      ...
      // loop columns in ccs
      for (j = 0; j < columns; j++) {
        // loop values for column
        for (k = sptr[j], n = sptr[j + 1]; k < n; k++) {
          // row
          i = sindex[k];
          // index in crs arrays
          var crsk = ptr[i]++;
          ...
          index[crsk] = j;
          // value
          values[crsk] = object.clone(svalues[k]);
        }
      }
      // shift back ptr
      for (i = rows; i > 0; i--)
  ...
  CrsMatrix.prototype = new Matrix();
    ...
    for (var i = kSub; i < rows && values.length < n; i++) {
      ...
      for (var x = k0; x < k1; x++) {
        ...
        if (j === i + kSuper - kSub) {
          ...
          // store column (always first column)
    ...
    for (var i = 0; i < rows; i++) {
      ...
      for (var z = 0; z < c; z++) {
        ...
        for (var k = k0; k < k1; k++) {
          ...
          var j = matrix._index[k];
          // multiply & aggregate
          value = add(value, multiply(matrix._values[k], get(j, z)));
        }
   ...
   */
  CrsMatrix.prototype.swapRows = function (i, j) {
    // check index
    ...
    // swap rows
    _swapRows(i, j, this._values, this._index, this._ptr);
    return this;
  };
   ...
   */
  CrsMatrix.prototype.lup = function () {
    // none of the LUP decomposition algorithms are optimized for CRS. Converting from CRS to CCS performs very whell...
    var m = new type.CcsMatrix(this);
    // use lup from ccs
    var r = m.lup();
    // return LUP
    return {
  ...
  var _swapRows = function (x, y, values, index, ptr) {
    // make sure x is before y
    if (x > y) {
      var t = x;
      x = y;
      y = t;
    }
    // find indeces for row x
    var kx0 = ptr[x];
    var kx1 = ptr[x + 1];
    var lx = kx1 - kx0;
    // find indeces for row y
    var ky0 = ptr[y];
    var ky1 = ptr[y + 1];
    var ly = ky1 - ky0;
    // vars
    var i, yvalues, yindex, xvalues, xindex;
    // splice row y
    yvalues = values.splice(ky0, ly);
    yindex = index.splice(ky0, ly);
    // splice row x
    xvalues = values.splice(kx0, lx);
    xindex = index.splice(kx0, lx);
    // insert row y
    for (i = 0; i < ly; i++) {
      values.splice(kx0 + i, 0, yvalues[i]);
      index.splice(kx0 + i, 0, yindex[i]);
    }
    // recalculate x index
    var k = ky0 - lx + ly;
    // insert row x
    for (i = 0; i < lx; i++) {
      values.splice(k + i, 0, xvalues[i]);
      index.splice(k + i, 0, xindex[i]);
    }
    // update ptr if needed
    if (lx !== ly) {
      // loop from x to y rows
      for (i = x + 1; i <= y; i++) {
        ptr[i] += -lx + ly;
      }
    }
   ...
   */
  CrsMatrix.prototype.forwardSubstitution = function (b) {
    // validate matrix and vector, initialize x as copy of column vector b
    var x = this._substitutionValidation(b);
    // rows & columns
    var rows = this._size[0];
    // matrix arrays
   ...
   * @return {Matrix}               A column vector with the linear system solution
   */
  CrsMatrix.prototype.backwardSubstitution = function (b) {
    // validate matrix and vector, initialize x as copy of column vector b
    var x = this._substitutionValidation(b);
    // rows & columns
    var rows = this._size[0];
