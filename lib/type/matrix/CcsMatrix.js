...
function factory (type, config, load, typed) {
  ...
  CcsMatrix.prototype.swapRows = function (i, j) {
    ...
    // swap rows
    _swapRows(i, j, this._size[1], this._values, this._index, this._ptr);
    ...
    for (j = 0; j < columns; j++) {
      ...
      if (j > 0) {
        ...
        spa.forEach(0, j - 1, function (k, vkj) {
          ...
          _forEachRow(k, lvalues, lindex, lptr, function (i, vik) {
            // check row is below k
            if (i > k) {
              // update spa value
              spa.accumulate(i, unaryMinus(multiply(vik, vkj)));
            }
          });
        });        
      }
