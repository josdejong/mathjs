...
function factory (type, config, load, typed) {
  ...
  CcsMatrix.prototype.lup = function () {
    ...
    // vars
    var i, j, k;
    // permutation vector & permutation vector indeces
    var pv = new Array(rows);
    var pvi = new Array(rows);
    for (i = 0; i < rows; i++) {
      pv[i] = i;
      pvi[i] = i;
    }
    // swap indices in permutation vector (condition x < y)!
