...
function factory (type, config, load, typed) {
    ...
    if (source.type === 'CcsMatrix') {
      ...
      for (i = 0; i < rows; i++) {
        ...
        for (k = sptr[i], n = sptr[i + 1]; k < n; k++) {
          ...
          index[ccsk] = i;
          // value
          values[ccsk] = object.clone(svalues[k]);
        }
      }
      // shift back ptr
      for (j = columns; j > 0; j--)
  ...
  CcsMatrix.prototype = new Matrix();
    ...
    for (var z = 0; z < c; z++) {
      ...
      for (var j = 0; j < columns; j++) {          
        ...
        for (var k = k0; k < k1; k++) {
          ...
          var i = matrix._index[k];
          // multiply & aggregate
          spa.accumulate(i, multiply(matrix._values[k], get(j, z)));
        }
      }
      // finished processing column z
   ...
   */
  CcsMatrix.prototype.swapRows = function (i, j) {
    // check index
    ...
    // swap rows
    CcsMatrix._swapRows(i, j, this._size[1], this._values, this._index, this._ptr);
   ...
   */
  CcsMatrix._forEachRow = function (j, values, index, ptr, callback) {
  ...
  /**
   * Swap rows x and y in CCS Matrix data structures.
   *
   ...
   * @param {Array} ptr        Matrix column pointers
   */
  CcsMatrix._swapRows = function (x, y, columns, values, index, ptr) {
    // loop columns
    for (var j = 0; j < columns; j++) {
      // k0 <= k < k1 where k0 = _ptr[j] && k1 = _ptr[j+1]
      var k0 = ptr[j];
  ...
  CcsMatrix.prototype.forwardSubstitution = function (b) {
    ...
    b = this._substitutionValidation(b);
    // rows & columns
    var rows = this._size[0];
    var columns = this._size[1];
    // matrix arrays
