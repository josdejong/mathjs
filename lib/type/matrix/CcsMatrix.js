...
function factory (type, config, load, typed) {
    ...
    for (var z = 0; z < c; z++) {
      ...
      for (var j = 0; j < columns; j++) {          
        ...
        for (var k = k0; k < k1; k++) {
          ...
          var i = matrix._index[k];
          // multiply & aggregate
          data[i] = add(data[i], multiply(matrix._values[k], get(j, z)));
        }          
      }
  ...
  CcsMatrix.prototype.lup = function () {
    ...
    var columns = this._size[1];
    // clone arrays
    var values = object.clone(this._values);
    var index = object.clone(this._index);
    var ptr = object.clone(this._ptr);
    // diagonal length
    var n = rows > columns ? columns : rows;
    // p (eye [n, n])
    var p = CcsMatrix.diagonal([n, n], 1, 0);
    // l (eye [rows, n])
    var l = CcsMatrix.diagonal([rows, n], 1, 0);
    // u (empty [n, columns])
    var u = _resize(new CcsMatrix(), n, columns);
    // vars
    var i, j, k;
    // loop columns
    for (j = 0; j < columns; j++) {      
      ...
      for (k = k0; k < k1 && j !== 0; k++) {
        // row
        i = index[k];
        // check row is in the upper diagonal matrix
        if (i < j) {
          // value @ [i, j]
          var vij = values[k];
          // loop other values in this column
          for (var ko = k + 1; ko < k1; ko++) {
            // row
            var r = index[ko];
            // loop values at column i
            var ki0 = ptr[i];
            var ki1 = ptr[i + 1];
            for (var ki = ki0; ki < ki1; ki++) {
              ...
              if (rki === r) {
                // update value
                values[ki] = subtract(values[ki], multiply(values[ko], vij));
              }
              else if (rki > r) {
                // exit loop
                break;
      ...
      // row with larger value in matrix @ column j
      var pi = 0;
      var pabsv = 0;
      var vjj = 0;
      // loop k within [k0, k1[
      for (k = k0; k < k1; k++) {
        // row index
        i = index[k];
        // check row is in the lower diagonal matrix
        if (i >= j) {
          // value & |value|
          var v = values[k];
          var absv = abs(v);
          // check value is greater than pivote value
      ...
      for (k = k0; k < k1; k++) {
        ...
        // check row is in the lower diagonal matrix
        if (i > j) {
          // update row value @ [i, j]
          values[k] = divideScalar(values[k], vjj);
        }
      }
    }
    return {
