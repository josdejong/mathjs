...
module.exports = function (math) {
  ...
  CcsMatrix.prototype.get = function (index, d) {
    if (!isArray(index))
   ...
   */
  CcsMatrix.prototype.map = function (callback, matrix, nonZeros) {
    // rows and columns
    var rows = this._size[0];
    var columns = this._size[1];
    // invoke callback
    var invoke = function (v, index) {
      // invoke callback
      return callback(v, index, matrix);
    };
    // invoke _map
  ...
  var _map = function (matrix, minRow, maxRow, minColumn, maxColumn, callback, zeros) {
    ...
    for (var j = minColumn; j <= maxColumn; j++) {
      ...
      // zero values
      for (var y = Math.max(p, minRow); zeros && y <= maxRow; y++)
   ...
   * @param {Matrix} matrix       The Matrix instance
   */
  CcsMatrix.prototype.forEach = function (callback, matrix) {
    // values index
    var k = 0;
    // rows and columns
    var rows = this._size[0];
