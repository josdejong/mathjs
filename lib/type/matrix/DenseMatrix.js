...
function factory (type, config, load, typed) {
  ...
  DenseMatrix.diagonal = function (size, value, k, defaultValue) {
    ...
    if (!defaultValue) {
      ...
      defaultValue = (_value(0) instanceof type.BigNumber) ? new type.BigNumber(0) : 0;  
    }
  ...
  var _multiplyVectorVector = function (matrix, m, get) {
    ...
    for (var i = 0; i < m; i++) {
      ...
      result = add(result, multiply(matrix._data[i], get(i)));
    }
    return result;
  };
  ...
  var _multiplyVectorMatrix = function (matrix, m, n, get) {
    ...
    for (var j = 0; j < n; j++) {
      ...
      for (var i = 0; i < m; i++) {
        // multiply and accumulate
        sum = add(sum, multiply(matrix._data[i], get(i, j)));
      }
      result[j] = sum;
    }
  ...
  var _multiplyMatrixVector = function (matrix, m, n, get) {
    ...
    for (var i = 0; i < m; i++) {
      ...
      for (var j = 0; j < n; j++) {
        // multiply & accumulate
        sum = add(sum, multiply(row[j], get(j)));
      }
      result[i] = sum;
    }
    // check we need to squeeze the result into a scalar
  ...
  var _multiplyMatrixMatrix = function (matrix, m, n, c, get) {
    ...
    for (var i = 0; i < m; i++) {
      ...
      for (var j = 0; j < c; j++) {
        ...
        for (var x = 0; x < n; x++) {
          // multiply & accumulate
          sum = add(sum, multiply(row[x], get(x, j)));
        }
        result[i][j] = sum;
      }
    }
  ...
  DenseMatrix.prototype.forwardSubstitution = function (b) {
    // validate dimensions
    if (this._size.length !== 2)
      throw new RangeError('Matrix must be two dimensional (size: ' + string.format(this._size) + ')');
    // rows & columns
    ...
    for (var i = 0; i < rows; i++) {
      ...
      for (var j = 0; j < i; j++) {
        // update x
        x[i] = subtract(x[i], multiply(data[i][j], x[j]));
      }
      x[i] = divideScalar(x[i], data[i][i]);
    }
    // return vector
    return new DenseMatrix({
      data: x,
  ...
  DenseMatrix.prototype.backwardSubstitution = function (b) {
    // validate dimensions
    if (this._size.length !== 2)
      throw new RangeError('Matrix must be two dimensional (size: ' + string.format(this._size) + ')');
    // rows & columns
