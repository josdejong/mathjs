  ...
  DenseMatrix.prototype.multiply = function (value) {
    ...
    if (value instanceof Matrix) {
      ...
      if (z.length === 1) {
        ...
        return _multiply(this, z[0], 1, true, function (i) {
          // value[i]
          return value.get([i]);
        });
      }
      ...
      if (z.length === 2) {        
        ...
        return _multiply(this, z[0], z[1], false, function (i, j) {
          // value[i, j]
    ...
    if (isArray(value)) {
      ...
      if (s.length === 1) {
        // mutiply matrix x vector array
        return _multiply(this, s[0], 1, true, function (i) {
          // value[i]
          return value[i];
        });
      }
      if (s.length === 2) {
        ...
        return _multiply(this, s[0], s[1], false, function (i, j) {
          // value[i, j]
          return value[i][j];
        });
      }
    ...
    return this.map(function(data) {
      return math.multiply(data, value);
    });
  };
  
  var _multiply = function (matrix, r, c, vector, get) {
    ...
    switch (matrix._size.length) {
      case 1:
        // vector
        rows = 1;
        columns = matrix._size[0];
        // row provider
        row = function () {
          return matrix._data;
        };
        // size
        size = [c];
        // indicate it is a vector
        vector = true;
        break;
      case 2:
        // m x n
        rows = matrix._size[0];
        columns = matrix._size[1];
        // row provider
        row = function (i) {
          return matrix._data[i];
        };
        // size
        size = [rows, c];
        break;
      default:
        throw new Error('Can only multiply a 1 or 2 dimensional matrix ' +
                        '(matrix has ' + matrix._size.length + ' dimensions)');
    }
    ...
    for (var i = 0; i < rows; i++) {
      ...
      for (var j = 0; j < c; j++) {
        ...
        for (var n = 0; n < columns; n++) {
          // multiply
          var p = math.multiply(x[n], get(n, j));
          // accumulate
    ...
    if (rows === 1 && c === 1)
      return vector ? res[0] : res[0][0];

    // return matrix
    return new DenseMatrix({
      data: vector ? res[0] : res,
      size: size
    });
  };
