...
function factory (type, config, load, typed) {
  ...
  function _createRange(arg) {
    ...
    switch (arg.length) {
      case 2:
        return new type.Range(arg[0], arg[1]); // start, end
      case 3:
        return new type.Range(arg[0], arg[1], arg[2]); // start, end, step
      default:
  ...
  Index.prototype.toString = function () {
    ...
    for (var i = 0, ii = this._ranges.length; i < ii; i++) {
      var range = this._ranges[i];
      var str = format(range.start);
      if (range.step != 1) {
        str += ':' + format(range.step);
      }
      str += ':' + format(range.end);
      strings.push(str);
    }
