...
var format = require('../../../util/number').format;
...
function factory (type) {
   ...
   *
   * Usage:
   *     var index = new Index(range1, range2, set1, set2, ...);
   *
  ...
  function Index() {
    ...
    for (var i = 0, ii = arguments.length; i < ii; i++) {
      ...
      else if (arg && (Array.isArray(arg) || arg.isMatrix)) {
        this._ranges.push(_createSet(arg.valueOf()));
        this._isScalar = false;
      }
      else if (typeof arg === 'number') {
        this._ranges.push(_createSet([arg]));
      }
      // TODO: implement support for wildcard '*'
      else {
        throw new TypeError('Ranges must be an Array, Matrix, Number, or Range');
      }
    }
  }
  ...
  Index.prototype.clone = function () {
    var index = new Index();
    index._ranges = clone(this._ranges);
  ...
  Index.prototype.toArray = function () {
    var array = [];
    for (var i = 0, ii = this._ranges.length; i < ii; i++) {
      array.push(this._ranges[i].toArray());
    }
    return array;
  };
  ...
  Index.prototype.toString = function () {
    ...
    for (var i = 0, ii = this._ranges.length; i < ii; i++) {
      strings.push(this._ranges[i].toString());
    }
