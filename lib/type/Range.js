...
/**
 * Get the string representation of the range, optionally rounding to a
 * specified number of digits. Output is formatted as 'start:step:end',
 * for example '2:6' or '0:0.2:11'
 * @param {Number} [precision]   Number of digits to round the numbers
 * @returns {String} str
 */
Range.prototype.format = function format(precision) {
  var str = number.format(this.start, precision);

  if (this.step != 1) {
    str += ':' + number.format(this.step, precision);
  }
  str += ':' + number.format(this.end, precision);
  return str;
};
