...
function mathjs (settings) {
  ...
  var _settings = {
    ...
    number: {
      ...
      'defaultType': 'number'
      // TODO: add more settings for BigNumber
  ...
  math.config = function config (settings) {
    if (settings) {
      ...
      // TODO: remove deprecated settings some day (deprecated since version 0.15.0)
      if (settings.precision) {
        throw new Error('_settingsion settings.precision is deprecated. ' +
            'Use math.format(value, precision) instead.')
      }
      if (settings.matrix && settings.matrix.default) {
        throw new Error('_settingsion matrix.default is deprecated. ' +
