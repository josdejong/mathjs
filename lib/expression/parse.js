...
function factory (type, config, load, typed) {
  ...
  var number_config = 'number'      // 'number', 'Fraction', or 'BigNumber'
  ...
  function parseNumber () {
    ...
    if (token_type === TOKENTYPE.NUMBER) {
      ...
      if (config.number === 'BigNumber') {
        return new ConstantNode(new type.BigNumber(numberStr));
      }
      else if (config.number === 'Fraction') {
        return new ConstantNode(new type.Fraction(numberStr));
      }
      else {
        // The following regexp is relatively permissive
        if (!/^[\-+]?((\d+\.?\d*)|(\d*\.?\d+))([eE][+\-]?\d+)?$/.test(numberStr)) {
          throw new Error('Invalid numeric value "' + numberStr + '"');
        }

        // remove leading zeros like '003.2' which are not allowed by JavaScript
        var number = parseFloat(numberStr.replace(/^(0*)[0-9]/, function (match, zeros) {
