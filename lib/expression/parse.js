...
function factory (type, config, load, typed) {
  ...
  function parseFraction () {
    ...
    while(true) {
      ...
      // A fraction must be "integer / integer"
      // TODO: Is there a better way to test for integers?
      if(token === '/' && type.isConstantNode(last) && Math.floor(parseFloat(last.value)) === parseFloat(last.value)) {
        
        // I think these are all the side effects of getTokenSkipNewline()
        var temp_token_type = token_type;
        var temp_token = token;
        var temp_comment = comment;
        var temp_index = index;
        var temp_c = c;
        
        // Look ahead to see if the next token is an integer
        getTokenSkipNewline();
        
        // TODO: Is there a better way to test for integers?
        if(token_type === TOKENTYPE.NUMBER && Math.floor(parseFloat(token)) === parseFloat(token)) { 
          last = parseUnary();
          node = new OperatorNode('/', 'divide', [node, last]);
        } 
        else {
          // Not a fraction, so rewind
