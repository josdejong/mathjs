...
function factory (type, config, load, typed) {
  ...
  function parseMultiplyDivide () {
    ...
    node = config.promoteImplicit ? parseImplicitMultiplication() : parseUnary();
    last = node;
    ...
    while (true) {
      if (operators.hasOwnProperty(token)) {
        ...
        // Parse implicit first, if it is promoted.
        last = config.promoteImplicit ? parseImplicitMultiplication() : parseUnary();
        node = new OperatorNode(name, fn, [node, last]);
      }
      else if (config.promoteImplicit) {
