...
function factory (type, config, load, typed) {
  ...
  // Set to true to promote the precedence of implicit multiplication above that of regular multiply and divide
  const promoteImplicit = true;
  ...
  function parseMultiplyDivide () {
    ...
    // Parse implicit first, if it is promoted.
    node = promoteImplicit ? parseImplicitMultiplication() : parseUnary();
    last = node;
    ...
    while (true) {
      if (operators.hasOwnProperty(token)) {
        ...
        // Parse implicit first, if it is promoted.
        last = promoteImplicit ? parseImplicitMultiplication() : parseUnary();
        node = new OperatorNode(name, fn, [node, last]);
      }
      else if (promoteImplicit) {
        // If promoteImplicit is set, then implicit multiplication was already handled in its own function.
        break;
      }
