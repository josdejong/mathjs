...
function IndexNode (math, object, ranges, paramScopes) {
  ...
  this.object = object;
  this.params = ranges; // TODO: rename to ranges
...
IndexNode.prototype._compile = function (defs) {
  ...
  var rangesUseEnd = this.params.map(function (range) {
  ...
  var ranges = this.params.map(function(range, i) {
  ...
  if (someUseEnd) {
    return '(function () {' +
        '  var size = math.size(' + this.object._compile(defs) + ').valueOf();' +
