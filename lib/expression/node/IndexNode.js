...
function factory (type, config, load, typed) {
  ...
  IndexNode.prototype._compile = function (defs, args) {
    ...
    var ranges = this.compileRanges(defs, childArgs);
    ...
    if (sizeNeeded) {
      return '(function () {' +
          ...
          '  return math.subset(obj, math.index(' + ranges + '));' +
          '})()';
    }
    else {
      return 'math.subset(' + object + ',' + 'math.index(' + ranges + '))';
    }
  };

  // TODO: comment and refactor
  IndexNode.prototype.compileReplacement = function (defs, args, contents, replacement) {
    var childArgs = Object.create(args);
    var ranges = this.compileRanges(defs, childArgs);
    ...
    if (!replacement) {
      replacement = '(function (obj) {' +
        contents +
        '})(math.subset(obj, index))';
    }

    var code =
        'var index = math.index(' + ranges + ');' +
        'return math.subset(obj, index, ' + replacement + ');';

    if (this.object.isIndexNode) {
      code = this.object.compileReplacement(defs, childArgs, code);
    }
   ...
   * @return {string} js
   */
  // TODO: let compileRanges return an object which denotes whether the matrix size is needed
  IndexNode.prototype.compileRanges = function (defs, args) {
