...
function factory (type, config, load, typed) {
  ...
  function UpdateNode(index, expr) {
    ...
    Object.defineProperty(this, 'name', {
      get: function () {
        return this._findSymbolNode().name;
      }.bind(this),
      set: function () {
  ...
  /**
   * Find the symbol node from the indexes
   * @return {SymbolNode} Symbol name
   * @private
   */
  UpdateNode.prototype._findSymbolNode = function () {
    return this.index.filter(function (node) {
  ...
  UpdateNode.prototype._compile = function (defs, args) {
    ...
    var symbol = this._findSymbolNode();
    ...
    if (indexNodes.length > 1 || compiledIndex.sizeNeeded) {
      ...
      var code = indexNodes.reduce(function (code, node) {
        ...
        var inner = code ? '(' + code + ')(math.subset(object, index))' : 'replacement';

        return 'function (object) {\n' +
            (compiledIndex.sizeNeeded ? 'var size = math.size(object).valueOf();\n' : '') +
            'var index = ' + index + ';\n' +
            'return math.subset(object, index, ' + inner + ');\n' +
            '}';
      }, null);

      return '(function () {' +
          'var replacement = ' + replacement + ';' +
          'scope["' + symbol.name + '"] = (' + code + ')(' + symbol._compile(defs, args) + ');' +
          'return replacement;' +
          '})()';
    }
    else {
