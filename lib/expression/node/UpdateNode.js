...
function factory (type, config, load, typed) {
  ...
  UpdateNode.prototype._compile = function (defs, args) {
    ...
    var ranges = this.index.compileRanges(defs, childArgs);
    var replacement = this.expr._compile(defs, args);
    var object = this.index.object._compile(defs, childArgs);

    if (this.index.isIndexNode || sizeNeeded) {
      // TODO: refactor
      var calculateSize = sizeNeeded ? '  var size = math.size(obj).valueOf();' : '';

      return 'scope["' + symbol.name + '"] = (function (obj) {' +
          calculateSize +
          this.index.compileReplacement(defs, childArgs, '', replacement) +
          '})(' + symbol._compile(defs, childArgs) + ')';
    }
    else {
      return 'math.subset(' + object + ', math.index(' + ranges + '), ' + replacement + ')';
    }
  };
