...
var register = require('./compile').register;
...
function factory (type, config, load, typed) {
  ...
  function compileAssignmentNode (node, defs, args) {
    ...
    if (!node.index) {
      ...
      return 'setSafeProperty(scope, "' + node.object.name + '", ' + value + ')';
    }
    else if (node.index.isObjectProperty()) {
      // apply an object property for example `a.b=2`
      return 'setSafeProperty(' + object + ', "' + node.index.getObjectProperty() + '", ' + value + ')';
    }
      ...
      return '(function () {' +
          ...
          '  var value = ' + value + ';' +
          '  ' + size +
          '  setSafeProperty(scope, "' + node.object.name + '", assign(object, ' + index + ', value));' +
          '  return value;' +
          '})()';
    }
      ...
      if (node.object.index.isObjectProperty()) {
        var parentProperty = '"' + node.object.index.getObjectProperty() + '"';
        return '(function () {' +
