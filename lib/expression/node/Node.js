...
Node.prototype.match = function () {
  throw new Error('Function Node.match is deprecated. See functions Node.filter and Node.replace.');
};
...
/**
 * Transform a node tree via a transform function.
 *
 ...
 *     });
 *
 * @param {function}  transformer  A transform function accepting a node, and returning
 *                              a replacement for the node or the original node.
 *                              Signature: replacer(node: Node) : Node
 * @return {Node} Returns the original node or its replacement
 */
Node.prototype.transform = function (transformer) {
  // TODO: first create a clone

  // check itself
  var res = transformer(this, null, null);

  // recurse over the child nodes
  this._traverse(function (node, index, parent) {
    var replacement = transformer(node, index, parent);
    if (index.indexOf('.') !== -1) {
      // traverse path
