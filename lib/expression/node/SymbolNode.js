...
function factory (type, config, load, typed, math) {
  ...
  function compileSymbolNode(node, defs, args) {
    ...
    // FIXME: validate symbol name, prevent injection of code
    ...
    if (args[node.name]) {
      ...
      return '("' + node.name + '" in scope ? getSafeProperty(scope, "' + node.name + '") : getSafeProperty(math, "' + node.name + '"))';
    }
    else {
      return '(' +
          '"' + node.name + '" in scope ? getSafeProperty(scope, "' + node.name + '") : ' +
          (Unit.isValuelessUnit(node.name) ?
          'new Unit(null, "' + node.name + '")' :
          'undef("' + node.name + '")') +
          ')';
    }
  }
