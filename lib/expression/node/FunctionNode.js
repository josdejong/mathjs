...
function factory (type, config, load, typed, math) {
   ...
   * @param {./Node | string} object   Object on which to invoke the arguments,
   *                          typically a SymboNode
   * @param {./Node[]} args
   */
  function FunctionNode(object, args) {
    ...
    if (typeof object === 'string') {
      ...
      object = new SymbolNode(object);
    }

    // validate input
    if (!object || !object.isNode) throw new TypeError('Node expected as parameter "object"');
    ...
    Object.defineProperty(this, 'name', {
      get: function () {
        return this.object.isSymbolNode ? this.object.name : 'anonymous';
      }.bind(this),
      set: function () {
        throw new Error('Cannot assign a new name, name is readonly');
      }
    })
  }
  ...
  FunctionNode.prototype._compile = function (defs, args) {
    ...
    if (this.object.isSymbolNode) {
      ...
      if (isRaw) {
        ...
        return object + '(' + argsName + ', math, scope)';
      }
      else {
        // "regular" evaluation
        return object + '(' + jsArgs.join(', ') + ')';
      }
    }
    else {
      ...
      return '(function () {' +
          'var object = ' + object + ';' +
          'return (object && object.rawArgs) ' +
          ' ? object(' + argsName + ', math, scope)' +
          ' : object(' + jsArgs.join(', ') + ')' +
          '})()';
    }
  };
  ...
  FunctionNode.prototype.map = function (callback) {
    var object = this.object.map(callback);
    var args = [];
    for (var i = 0; i < this.args.length; i++) {
      args[i] = this._ifNode(callback(this.args[i], 'args[' + i + ']', this));
    }
    return new FunctionNode(object, args);
  };
  ...
  FunctionNode.prototype.clone = function () {
    return new FunctionNode(this.object, this.args.slice(0));
  };
  ...
  FunctionNode.prototype._toString = function (options) {
    ...
    return this.object.toString() + '(' + this.args.join(', ') + ')';
  };
