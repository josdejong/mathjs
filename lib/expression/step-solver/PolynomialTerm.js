...
PolynomialTerm.isPolynomialTerm = function(node, allowCoeff = true,
  ...
  if (NodeType.operator(node)) {
    if (node.op === "^") {
      if (!NodeType.symbol(node.args[0])) {
        return false;
      }
      const exponentNode = node.args[1];
      if (onlyConstantExponent) {
        return NodeType.constant(exponentNode);
      } else {
    ...
    if (node.op === "*") {
      return (allowCoeff &&
              node.implicit &&
              NodeType.constant(node.args[0]) && // the coefficient
              PolynomialTerm.isPolynomialTerm(node.args[1], false));
    }
    else if (node.op === "/") {
      return (allowCoeff &&
              NodeType.constant(node.args[1]) && // the denominator
  ...
  else if (NodeType.parenthesis(node) || NodeType.constant(node)) {
    return false;
  }
  else if (NodeType.symbol(node)) {
    return true;
...
PolynomialTerm.multiplyConstantAndPolynomialTerm = function(node) {
  ...
  if (NodeType.constant(node.args[0])) {
  ...
  // If there was no coefficient, the constant becomes the coefficient
  else {
...
PolynomialTerm.simplifyPolynomialFraction = function(node) {
  ...
  if (newNumeratorCoeff === 1) {
    if (NodeType.unaryMinus(numeratorTerm)) {
...
function canMultiplyConstantAndPolynomial(node) {
  ...
  return ((PolynomialTerm.isPolynomialTerm(node.args[0], false) &&
            NodeType.constant(node.args[1])) ||
          (PolynomialTerm.isPolynomialTerm(node.args[1], false) &&
            NodeType.constant(node.args[0])));
}
...
function canSimplifyPolynomialFraction (node) {
  ...
  // the denominator of a fraction should never be negative
  // either the numerator is negative or neither is
  if (polyTerm.denominatorCoeff < 0) {
    return true;
  }
