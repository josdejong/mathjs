  ...
  switch (node.type) {
    ...
    case 'OperatorNode':
      ...
      if (node.op === "+" || node.op === "*" || node.op === "/") {
        ...
        return flattenSupportedOperation(node, op);
      }
...
// Flattens operations (see flattenOps docstring) for an operator node
// with an operation type that can be flattened. Currently * + / are supported.
// TODO: support subtraction
function flattenSupportedOperation(node, op) {
  let hasChanged = false;
  let changeType = MathChangeTypes.NO_CHANGE;

  // STEP 1: get the list of operands that this operator operates on.
  // e.g. 2 + 3 + 4 + 5 is stored as (((2 + 3) + 4) + 5) in the tree and we
  // want to get the list [2, 3, 4, 5]
  let opStatus = getOperands(node, op);
  // Sometimes the tree changes when we flatten, in a way we'd want to count
  // as a step.
  ...
  if (operands.length === 1) {
    ...
    // Check for this by seeing if this is a division node with division in the
    // arguments (which should never happen, 2/3/4 would become 2/(3*4))
    if (node.op === "/" && operands.some(node => node.op && node.op === "/")) {
      node = NodeCreator.operator("*", operands);
    } else {
      node.args = operands; // otherwise keep the operator, replace operands
    }
...
function getOperands(node, op) {
  ...
  if (node.type !== 'OperatorNode') {
    return flattenOpsReturnOperandStatus(node);
  }
  switch (node.op) {
...
// Flattens operations for `node` and returns a list of operands in an
// OperandsStatus object, where there is only one operand: the flattened node.
function flattenOpsReturnOperandStatus(node) {
