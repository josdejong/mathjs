...
// Input must be a top level expression, output is a NodeStatus object.
function removeUnnecessaryParens(node) {
  // Parens that wrap everything are redundant
  // note that removeUnnecessaryParensDFS recursively removes parens that aren't
...
// Recursively moves any parenthesis around nodes that can't be resolved further,
// if it doesn't change the value of the expression. Returns a NodeStatus object.
...
function removeUnnecessaryParensInParenthesisNode(node) {
  ...
  else if (NodeType.unaryMinus(node.content)) {
    nodeStatus = removeUnnecessaryParensDFS(node.content);
    if (nodeStatus.hasChanged) {
      hasChanged = true;
      node.content = nodeStatus.node;
