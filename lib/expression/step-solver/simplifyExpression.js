...
function stepThrough(node, debug=false) {
  if (debug) {
    console.log('\n\nSimplifying: ' + prettyPrint(node, true));
  }
  ...
  while (nodeStatus.hasChanged) {
    ...
    if (iters++ == MAX_STEP_COUNT) {
      throw Error('Potential infinite loop for equation: ' + originalExpressionStr);
    }
  }
  ...
  if (steps.length > 0) {
    steps[steps.length - 1].exprString = prettyPrint(nodeStatus.node);
...
function addStep(steps, nodeStatus, debug) {
  ...
  let expression = prettyPrint(nodeStatus.node);
  steps.push({
...
function plusMinusToMinus(node) {
  ...
  // Then look for + -
  const exprString = prettyPrint(nodeStatus.node, true);
