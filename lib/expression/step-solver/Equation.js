...
class Equation {
  ...
  prettyPrint(showPlusMinus=false, latex=false) {
    let leftSide;
    let rightSide;
    if (latex) {
      leftSide = this.leftNode.toTex();
      rightSide = this.rightNode.toTex();
    } else {
      leftSide = prettyPrintNode(this.leftNode, showPlusMinus);
      rightSide = prettyPrintNode(this.rightNode, showPlusMinus);
    }
