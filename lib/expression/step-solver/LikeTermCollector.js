...
function collectLikeTermsDFS(rootNodeObj, node) {
  switch (node.type) {
      case 'OperatorNode':
        ...
        if (canCollectLikeTerms(node)) {
          // TODO: this is a super hacky way of dealing with the root node. this whole setup is changing soon,
          // I just want to do it in a different diff than this
          if (node.toString().replace(/\s/g, "") === rootNodeObj.expr.toString().replace(/\s/g, "")) {
            rootNodeObj.expr = collectLikeTerms(node);
          } else {
