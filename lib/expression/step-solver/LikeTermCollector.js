...
function getTermsForCollectingAddition(node) {
  ...
  for(let i=0; i < node.args.length; i++) {
    ...
    if (PolynomialTerm.isPolynomialTerm(child)) {
      ...
      terms = Util.appendToListInDictionary(terms, termName, child);
    }
    else {
      switch(child.type) {
        case 'ConstantNode':
          Util.appendToListInDictionary(terms, CONSTANT, child);
          break;
        case 'OperatorNode':
        case 'ParenthesisNode':
          Util.appendToListInDictionary(terms, OTHER, child);
          break;
        default:
...
function getTermsForCollectingMultiplication(node) {
  ...
  for(let i=0; i < node.args.length; i++) {
    ...
    if (PolynomialTerm.isPolynomialTerm(child)) {
      ...
      switch(child.type) {
        case 'ConstantNode':
          terms = Util.appendToListInDictionary(terms, CONSTANT, child);
          break;
        // It would be an unsupported operator if it hasn't been collected yet
        case 'OperatorNode':
        case 'ParenthesisNode':
          terms = Util.appendToListInDictionary(terms, OTHER, child);
          break;
        default:
          // Note that we shouldn't get any symbol nodes in the switch statement
          // since they would have been handled by isPolynomialTerm
...
function addToTermsforPolynomialMultiplication(terms, node) {
  ...
  if (polyTerm.coeff === 1) {
    ...
    terms = Util.appendToListInDictionary(terms, termName, node);
  }

  // If there's a denominator, getting the symbolic term is different
  else if (polyTerm.denominatorCoeff) {
    let coeffTerm;
    // check if the coeficient is a fraction
    if (!polyTerm.denominatorCoeff || polyTerm.denominatorCoeff == 1) {
    ...
    terms = Util.appendToListInDictionary(terms, CONSTANT, coeffTerm);
    ...
    terms = Util.appendToListInDictionary(terms, termName, symbolicTerm);
  }
