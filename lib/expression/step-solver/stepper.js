    ...
    case 'ParenthesisNode':
      switch (node.content.type) {
        ...
        case 'OperatorNode':
          ...
          break;
        // If the content is also parens, we have doubly nested parens, so
        // delete one. Then recuse.
        case 'ParenthesisNode':
  ...
  for(let i=0; i < node.args.length; i++) {
    ...
    switch(child.type) {
      case 'ConstantNode':
        ...
        break;
      // TODO: add support for exponents and coefficients on symbols
      case 'SymbolNode':
        if (terms[child.name]) {
          terms[child.name].push(child);
        } else terms[child.name] = [child];
        break;
      case 'OperatorNode':
      case 'ParenthesisNode':
        if (terms['other']) {
