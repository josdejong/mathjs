...
function expandOp(op, node) {
  if (node.type === 'OperatorNode' && node.op === op) {
    ...
    // We consider this to be possible when there is an implicit multiplication
    // (e.g. 2x is implicit, 2*x is not)
    if (op == "*" && node.implicit && node.args.length === 2 &&
          isPolynomialTerm(node.args[1], false)) {
