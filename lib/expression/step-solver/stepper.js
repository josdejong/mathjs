...
// For storing symbolic terms and performing operations on them.
// Has a name and an exponent.
// TODO: coefficients
class symbolicTermObj {
  constructor(node) {
    if (!isSymbolicTerm(node)) {
      this.name = '';
      this.exp = 0;
      return;
    }
    switch (node.type) {
      case 'OperatorNode':
        // this means it's of the same form as x^2
...
function collectLikeTerms(node) {
  ...
  for(let i=0; i < node.args.length; i++) {
    ...
    if (isSymbolicTerm(child)) {
      const termName = (new symbolicTermObj(child).name +
                        new symbolicTermObj(child).exp.toString());
      if (terms[termName]) {
    ...
    switch(child.type) {
      ...
      default:
        ...
        // since they would have been handled by isSymbolicTerm
