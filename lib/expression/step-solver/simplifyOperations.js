...
function performArithmetic(node) {
  ...
  // Only resolve division if we get an integer result.
  if (node.op === '/') {
    const numeratorValue = parseFloat(node.args[0]);
    const denominatorValue = parseFloat(node.args[1]);
...
function reduceExponentByZero(node) {
  ...
  if (NodeType.isConstant(exponent) && exponent.value === '0') {
    ...
    const nodeString = Util.prettyPrint(simplifiedNode)
