...
function simplifyOperationsDFS(node) {
  if (NodeType.parenthesis(node)) {
    ...
    const contentNodeStatus = simplifyOperationsDFS(node.content);
    ...
    // Start with recursing, so we simplify deeper nodes first
    const contentNodeStatus = simplifyOperationsDFS(node.args[0]);
    ...
    for (let i = 0; i < node.args.length; i++) {
      const argNodeStatus = simplifyOperationsDFS(node.args[i]);
      // always update args, since there might be changes that don't count as a step
...
function simplifyDoubleUnaryMinus(node) {
  if (!NodeType.unaryMinus(node)) {
    return new NodeStatus(node);
  }
  const unaryArg = node.args[0];
  // e.g. in - -x, -x is the unary arg
  if (NodeType.unaryMinus(unaryArg)) {
    node = unaryArg.args[0];
    return new NodeStatus(node, true, MathChangeTypes.RESOLVE_DOUBLE_UNARY_MINUS);
  }
  // e.g. - -4, -4 could be a constant with negative value
  else if (NodeType.constant(unaryArg)) {
    if (unaryArg.value < 0) {
      unaryArg.value *= -1;
      node = unaryArg;
      return new NodeStatus(node, true, MathChangeTypes.RESOLVE_DOUBLE_UNARY_MINUS);
  ...
  // e.g. -(-(5+2))
  else if (NodeType.parenthesis(unaryArg)) {
    let parenthesisNode = unaryArg;
    const parenthesisContent = parenthesisNode.content;
    if (NodeType.unaryMinus(parenthesisContent)) {
      // removes the unary minus after the open parenthesis
      parenthesisNode.content = parenthesisContent.args[0];
      // removes the unary minus before the open parenthesis
...
function performArithmetic(node) {
  if (NodeType.operator(node) && node.args.every(child => NodeType.constant(child))) {
...
function removeAdditionByZero(node) {
  if (node.op === "+") {
