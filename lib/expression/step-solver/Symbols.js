...
Symbols.getLastNonSymbolTerm = function(node, symbolName) {
  const symbolCoefficient = Symbols.getSymbolCoefficient(node, symbolName);
  if (symbolCoefficient) {
    return symbolCoefficient;
  }
  else if (NodeType.isOperator(node)) {
    for (let i = node.args.length - 1; i >= 0 ; i--) {
      const child = node.args[i];
      const nonSymbolTerm = Symbols.getNonSymbolTerm(child, symbolName);
      if (nonSymbolTerm) {
...
// Given a node containing a symbolName, will return the coefficient of the
// symbol if it exists
Symbols.getSymbolCoefficient = function(node, symbolName) {
  if (PolynomialTermNode.isPolynomialTerm(node)) {
    const polyTerm = new PolynomialTermNode(node);
    if (polyTerm.symbolName() === symbolName) {
...
Symbols.getNonSymbolTerm = function(node, symbolName) {
