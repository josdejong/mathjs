...
EquationOperations.removeSymbolFromRightSide = function(equation, symbolName) {
  ...
  if (symbolTerm){
    if (PolynomialTermNode.isPolynomialTerm(equation.rightNode)) {
    ...
    else if (NodeType.isOperator(equation.rightNode)) {
      ...
      throw Error('Unsupported node type: ' + equation.rightNode.type);
    }
...
EquationOperations.isolateSymbolOnLeftSide = function(equation, symbolName) {
  ...
  if (nonSymbolTerm) {
    if (NodeType.isOperator(equation.leftNode)) {
      ...
      throw Error('Unsupported node type: ' + equation.leftNode.type);
    }
