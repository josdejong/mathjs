    ...
    if (NodeType.isOperator(equation.rightNode)) {
      if (equation.rightNode.op === '+') {
        return performTermOperationOnEquation(
          equation, '-', symbolTerm, MathChangeTypes.SUBTRACT_FROM_BOTH_SIDES)
      }
      else {
      ...
      return performTermOperationOnEquation(
        equation, '+', symbolTerm.args[0], MathChangeTypes.ADD_TO_BOTH_SIDES)
    }
  }
...
EquationOperations.isolateSymbolOnLeftSide = function(equation, symbolName) {
  ...
  if (nonSymbolTerm) {
    if (NodeType.isOperator(equation.leftNode)) {
      ...
      return performTermOperationOnEquation(equation, inverseOp, nonSymbolTerm,
                                            changeType)
    }
  }
