...
EquationOperations.ensureSymbolInLeftNode = function(equation, symbolName) {
  const leftSideSymbolTerm = Symbols.getFirstTermWithSymbolName(
    equation.leftNode, symbolName);
  const rightSideSymbolTerm = Symbols.getFirstTermWithSymbolName(
...
EquationOperations.removeSymbolFromRightSide = function(equation, symbolName) {
  const symbolTerm = Symbols.getFirstTermWithSymbolName(
    equation.rightNode, symbolName);
  ...
  if (symbolTerm){
    if (NodeType.isOperator(equation.rightNode)) {
      if (equation.rightNode.op === '+') {
        newLeftNode = NodeCreator.operator(
          '-', [equation.leftNode, symbolTerm]);
        newRightNode = NodeCreator.operator(
          '-', [equation.rightNode, symbolTerm]);
        equation = new Equation(newLeftNode, newRightNode, equation.comparator);

        return new EquationStatus(
          equation, true, MathChangeTypes.SUBTRACT_FROM_BOTH_SIDES);
      }
        ...
        newRightNode = NodeCreator.operator(
          '/', [equation.rightNode, symbolTerm]);
        equation = new Equation(
          newLeftNode, newRightNode, equation.comparator);

        return new EquationStatus(
          equation, true, MathChangeTypes.DIVIDE_FROM_BOTH_SIDES);
      }
      else if (equation.rightNode.op === '/') {
        newLeftNode = NodeCreator.operator(
          '*', [equation.leftNode, symbolTerm]);
        ...
        return new EquationStatus(
          equation, true, MathChangeTypes.MULTIPLY_TO_BOTH_SIDES);
      }
       else {
      ...
      newLeftNode = NodeCreator.operator(
        '+', [equation.leftNode, symbolTerm.args[0]]);
      newRightNode = NodeCreator.operator(
        '+', [equation.rightNode, symbolTerm.args[0]]);
      equation = new Equation(newLeftNode, newRightNode, equation.comparator);

      return new EquationStatus(
        equation, true, MathChangeTypes.ADD_TO_BOTH_SIDES);
    }
  }
...
EquationOperations.isolateSymbolOnLeftSide = function(equation, symbolName) {
  let nonSymbolTerm = Symbols.getLastTermWithoutSymbolName(
    equation.leftNode, symbolName);
  let newLeftNode, newRightNode;
  ...
  if (nonSymbolTerm) {
    if (NodeType.isOperator(equation.leftNode)) {
      if (equation.leftNode.op === '+') {
        newLeftNode = NodeCreator.operator(
          '-', [equation.leftNode, nonSymbolTerm]);
        newRightNode = NodeCreator.operator(
          '-', [equation.rightNode, nonSymbolTerm]);
        equation = new Equation(
          newLeftNode, newRightNode, equation.comparator)
        return new EquationStatus(
          equation, true, MathChangeTypes.SUBTRACT_FROM_BOTH_SIDES);
        ...
        // if it's a constant fraction, multiply by the inverse fraction
        if (NodeType.isConstantFraction(nonSymbolTerm)) {
          const numeratorValue = parseFloat(nonSymbolTerm.args[0].value);
          const denominatorValue = parseFloat(nonSymbolTerm.args[1].value);
          const newNumeratorNode = NodeCreator.constant(denominatorValue);
          const newDenominatorNode = NodeCreator.constant(numeratorValue);
          nonSymbolTerm = NodeCreator.operator(
      ...
      newLeftNode = NodeCreator.operator(
        '+', [equation.leftNode, nonSymbolTerm.args[0]]);
      newRightNode = NodeCreator.operator(
        '+', [equation.rightNode, nonSymbolTerm.args[0]]);
      equation = new Equation(newLeftNode, newRightNode, equation.comparator)
      return new EquationStatus(
        equation, true, MathChangeTypes.ADD_TO_BOTH_SIDES);
    }
