...
function flattenOperands(node) {
  if (NodeType.constant(node, true)) {
    ...
    if ("+-/*".includes(node.op)) {
      let op;
      if (node.op === "/") {
        op = "*"; // Division is flattened in partner with multiplication
      } else if (node.op === "-") {
        op = "+"; // Subtraction is flattened in partner with addition
      } else {
        op = node.op;
      }
...
function flattenSupportedOperation(node, op) {
  ...
  let operands = getOperands(node, op);
...
// returns the list of all the node operated on by `op`
function getOperands(node, op) {
  ...
  if (op === '*' && isPolynomialTermMultiplication(node)) {
    ...
    let operands = [
      getOperands(node.args[0], op),
      // this operation will become addition, with the second operand wrapped
      // in a unary minus
      getOperands(NodeCreator.unaryMinus(node.args[1]), op)
    ];
...
function isPolynomialTermMultiplication(node) {
  ...
  if (NodeType.symbol(secondOperand)) {
    return true;
  }
  if (NodeType.operator(secondOperand)) {
...
function maybeFlattenPolynomialTerm(node) {
  ...
  if (NodeType.constant(lastOperand)) {
