...
module.exports = function (math) {
   ...
   *     sqrt, pow
   *
   * @param {Number | BigNumber | Boolean | Array | Matrix | Complex | null} a
  ...
  /**
   * Calculate the nth Root of a Complex number using De Moviers theorem
   * @param  {Complex} a
   * @param  {Number} root
   * @return {Array}  returns an array of n Polar form complex numbers representing the n roots.
   */
  function _nthRootComplex(a, root) {
    var _root = (root != undefined ? root : 2);
    var polar = a.toPolar();
    var roots = [];
    var r = Math.pow(polar.r, 1/_root);

    for(var k = 0; k < _root; k++) {
      roots.push({r: r, phi: (polar.phi + 2 * Math.PI * k)/_root});
    }
