...
function factory (type, config, load, typed) {
  ...
  function _sinh (x) {
    if (Math.abs(x) < 1 && x !==  0) {
      x = new type.BigNumber(x + '')
      return taylorExpansion(x, 17).toNumber();
    } else {
  ...
  function taylorExpansion(x, n){
    ...
    return SummationFunc(x, n).plus(taylorExpansion(x, n-2));
  }
  ...
  function SummationFunc(x, n) {
    var numerator = x.pow(n);
    var denominator = new type.BigNumber(math.factorial(n));
