...
function _sinh (x) {
  if (Math.abs(x) < 1) {
    return math.number(taylorExpansion2(x, 17));
  } else {
    return (Math.exp(x) - Math.exp(-x)) / 2;
  }
}

function SummationFn(x, n) {

  var numerator = bn(x).pow(n);
  var denominator = bn(trampoline(factorial.bind(null, n)));

  return math.divide(numerator, denominator)
}

function taylorExpansion2(x, n){

  if (n === 1) {
    return bn(x);
  }
  return math.add(taylorSinhFn2(x, n), taylorExpansion2(x, n-2));
}
