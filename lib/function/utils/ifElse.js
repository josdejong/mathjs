module.exports = function (math) {
  ...
  math.ifElse = function ifElse(condition, trueExpr, falseExpr) {
    if (arguments.length != 3) {
      throw new util.error.ArgumentsError('ifElse', arguments.length, 3);
    }
    ...
    throw new math.error.UnsupportedTypeError('ifElse', condition);
  };
  ...
  function _ifElseCollection(condition, trueExpr, falseExpr) {
    ...
    if (isCollection(trueExpr)) {
      ...
      if (!deepEqual(condition.size(), trueExpr.size())) {
        throw new RangeError('Dimension mismatch ([' +
            ...
            trueExpr.size().join(', ')
            + '])');
        throw new util.error.DimensionError(condition.size(), trueExpr.size());
      }
    }
    ...
    if (isCollection(falseExpr)) {
      ...
      if (!deepEqual(condition.size(), falseExpr.size())) {
        throw new util.error.DimensionError(condition.size(), falseExpr.size());
      }
    }
