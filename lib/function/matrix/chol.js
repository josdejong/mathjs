...
module.exports = function (math) {
   ...
   *  math.chol(A) produces an upper triangular R so that R'*R = A.
        If A is not positive definite, complex element is returned.
  ...
  math.chol = function chol(x, p) {
    ...
    if (size.length !== 2 || size[0] !== size[1]) {
      throw new Error('Matrix or array must be a NxN square matrix.');
    }
    ...
    if (p === 'lower'){
      ...
      for(var k=1; k<n; k++){
        ...
        akk = math.clone(array[k][k]);

        lk = math.multiply(math.inv(Lk), ak);
        lkt = math.transpose(lk);

        A[k][k] = math.sqrt(math.subtract(akk, math.multiply(lkt, lk)));
      ...
      throw new Error('Invalid option, the value should be upper or lower.');
    }
