...
function factory (type, config, load, typed) {
    ...
    'Matrix': function (A) {
      ...
      for(var i=1; i<q; i++) {
        if(i>1) {
          ...
          Aneg_to_i = multiply(Aneg_to_i, Aneg);
        }
        var factor = factorial(2*q-i) * factorial(q) / (factorial(2*q) * factorial(i) * factorial(q-i))
        
        N = add(N, multiply(factor, Apos_to_i));
        D = add(D, multiply(factor, Aneg_to_i));
      }
  ...
  function errorEstimate(infNorm, q, j) {
    var qfac = factorial(q);
    var twoqfac = factorial(2*q);
    var twoqp1fac = factorial(2*q+1);
    return 8.0 *
