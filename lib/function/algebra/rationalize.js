...
function factory (type, config, load, typed) {
   ...
   *     ret = math.rationalize('x+x+x+y',{},true) => ret.expression=3*x+y,  ret.variables = ["x","y"]
   *     ret = math.rationalize('-2+5x^2',{},true) => ret.expression=5*x^2-2,  ret.variables = ["x"], ret.coefficients=[-2,0,5]
   ...
   * @param  {Boolean}  complete   optional True if return an object, false if return expression node (default) 
   *
   ...
   *              {Expression Node} denominator: simplified denominator of expression
   ...
   */
   var rationalize = typed('rationalize', {
       ...
       'string, Object, boolean': function (expr, scope, complete) {
         return rationalize(parse(expr), scope, complete);
       },
       ...
       'Node, Object, boolean': function (expr, scope, complete) {

         var polyRet = polynomial(node, scope, true)  // Check if expression is a rationalizable polynomial
         ...
         if (nVars==1)
           if (expr.type==='OperatorNode'  &&  expr.op==='/')  {   // Separate numerator from denominator
              ...
              if (complete) { 
                 ...
                 retRationalize.denominator = ConstantNode(1);
              }
           }
           if (! complete)
             return expr;
           ...
           retRationalize.variables = variables;
