...
function factory (type, config, load, typed) {
  var polynomial = load(require('./getPolynomial'));
   ...
   *     rationalize(expr,scope,variables)
   *     rationalize(expr,scope,variables,extended)
   *
   * Examples:
   *
   ...
   *     math.rationalize('x+x+x+y',{},variables) = 3*x+y, variables => ["x","y"]
   *     math.rationalize('-2+5x^2',{},variables,coefficients) =  5*x^2-2,  variables => ["x"], coefficients=[-2,0,5]
   ...
   * @param  (Object}  scope  optional scope of expression or true for rational expression simplify
   ...
   */
   var rationalize = typed('rationalize', {
       'string': function (expr) {
         return rationalize(parse(expr), {}, [] ,[]);
       },

       'string, Object': function (expr, scope) {
         return rationalize(parse(expr), scope, [], []);
       },

       'string, Object, Array': function (expr, scope, variables) {
         return rationalize(parse(expr), scope, variables, []);
       },

       'string, Object, Array, Array': function(expr, scope, variables, coefficients) {
         return rationalize(parse(expr), scope, variables, coefficients);
       },
       ...
       'Node, Object': function (expr, scope) {
         return rationalize(expr, scope, [], []);
       },

       'Node, Object, Array': function (expr, scope, variables) {
         return rationalize(expr, scope, variables, []);
       },

       'Node, Object, Array, Array': function(expr, scope, variables, coefficients) {

         var node = polynomial(node, scope, variables, true)  // Check if expression is a rationalizable polynomial
         ...
         if (nVars==1)
           if (node.type==='OperatorNode'  &&  node.op==='/')  {   // Separate numerator from denominator
              node.args[0] = polyToCanonical(node.args[0],coefficients);
              node.args[1] = polyToCanonical(node.args[1]);
           } else 
              node = polyToCanonical(node,coefficients);
