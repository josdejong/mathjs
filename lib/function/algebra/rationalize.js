...
////////////////// Debug Code ///////////////////////////
   ...
   // if type = "..." is a set of characters to advance to avoid 
   ...
   function breakStr(s, len, type="") {  
     ...
     var pos = 0;
     while (pos <= n-1) {
       x += (isComm ? "// ": "") + s.slice(pos,pos+len) + "\n" ;
       pos += len;
       if (! isComm && (type!=="")) {
        for (var i=0;i<type.length;i++) {
          var prox = s.indexOf(type[i],pos);  // próxima vírgula 
          if (prox!==-1) {
            x += s.slice(pos,prox+1);
            pos += prox+1-pos;
            break    
          }
        } // for char options
   ...
   function funcCommon(func,expr,rules,lin) {
     ...
     if (prev === "") {
       prev = expr;
       glob += breakStr(count + "> Initial Expression: " + expr + "  before simplify call(" + lin + ")",len,"#") ;
       glob += breakStr("var expr = " + expr + ";",len);   // Expressão inicial 
     } else {
       actual = retStr(expr);  
       if (prev === actual) 
         glob += "// " + count + "> No expression change before simplify call(" + lin + ")\n" ;
       else
         glob += breakStr(count + "> Expression in simplify call [" + prev + "] changes to " + actual,len,"#");
     }
     ...
     prev = retStr(node);
     glob +=   breakStr(count + "> Expression in simplify call (" + lin + ") changes to " + prev,len,"#");
     glob +=   "// ==========================\n";
     return node;
   }
