...
////////////////// Debug Code ///////////////////////////
    ...
    function simplifyDeb(expr,second) {  ///XXX
    var actual; 
    count += 1;
    if (prev === "") {
       prev = expr;
       glob += count + "> Initial Expression: " + expr + "  before simplify call(" + __line + ")\n" ;
    } else {
      actual = retStr(expr);  
      if (prev === actual) 
        glob += count + "> No expression change before simplify call(" + __line + ")\n" ;
      else
        glob += count + "> Expression in simplify call [" + prev + "] changes to " + actual + "\n";
    }
    var node = simplify(expr,second);
    prev = retStr(node);
    glob +=   count + "> Expression in simplify call (" + __line + ") changes to " + prev + "\n";
    glob +=   "==========================\n";
    return node;
  }
  ...
  function parseDeb(expr) {  ///XXX
    ...
    if (prev === "") {  
      prev = expr;
      glob += count + "> Initial Expression: " + expr + "  before parse call(" + __line + ")\n" ;
    } else {
      actual = retStr(expr);  
      if (prev === actual) 
        glob += count + "> No expression change before parse call (" + __line + ")\n" ;
      else
        glob += count + "> Expression in parse call [" + prev + "] changes to " + actual + "\n";
    }
    var node = parse(expr);
    prev = retStr(node);
    glob +=   count + "> Expression in parse call (" + __line + ") changes to " + prev + "\n";
    glob +=   "==========================\n";
    return node;
  }
////////////////// End of Debug Code ///////////////////////////
