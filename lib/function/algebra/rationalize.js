...
function factory (type, config, load, typed) {
  ...
  var ob={ko:0};  //XXX  (Create an object to count and acum messages)
...
//      console.log(expr.toString());
      ...
      var debug=expr.toString().replace(/ /g,'');  //ZZZ 
      var edebug = debug.slice(0,4)==="x/(1";     //ZZZ Troublemaker expression
      ...
      if (nVars>=1) {   // If expression in not a constant
        ...
        if (edebug) appen(setRules.firstRules,"Inic+Succ" ,ob ,expr.toString())  //XXX                      
        ...
        while (true) {                  // Apply alternately  successive division rules and distr.div.rules
          ...
          if (edebug) appen(rules, eDistrDiv ? "DistrDiv" : "Succ", ob ,expr.toString())  //XXX                      
        ...
        if (redoInic)  {         // Apply first rules again without succ div rules (if there are changes)
             if (edebug) appen(setRules.firstRulesAgain, "ReInic", ob,expr.toString());  //XXX
             expr = simplify(expr,setRules.firstRulesAgain);   
        }

        if (edebug) appen(setRules.finalRules, "Final",ob,expr.toString());  //XXX
      ...
      if (edebug)   {      //ZZZ
        console.log("// =================================================");   //XXX
      // striComm('s',expr,60)                                               //XXX 
        s = striComm('Expression pre Canonical is ', expr,60,true)             //XXX   
        console.log(s);                                                        //XXX
        throw new ArgumentsError(s)                  
      }
 ...
 * @param  {boolean} silent    true when don't print nothing in console. 
 *
 *
 * @return string              expression divided is small pieces
 */
function striComm(varName, s, size, silent=false) {   //XXX
var saida="";
var piece;
 ...
 *
 * Syntax
 *    appen(vo,mensag, objCount, stringExpr)
 *
 * @param  {array|object} arrOrObj      scope (object) or rules (array)
 * @param  {string}  messag             custom message 
 * @param  {object}  objCount           object for refence counting
 * @param  {object|string} stringExpr   node or string expression
 *
 *
 * @return nothing 
 */
function appen(arrOrObj, messag, objCount, stringExpr) {  //XXX
var isArr = Array.isArray(arrOrObj);
objCount.ko ++;  
console.log("// ========================" + objCount.ko + "(" + messag + "): =========================");
process.stdout.write("vo = "); 
console.log(arrOrObj);
striComm('s',stringExpr,60);
console.log("expr = simplify(s,vo);");
}
 ...
 */             
  ...
  function polynomial (expr, scope, extended) {
    var variables = [];
    appen(scope,"Scope" ,ob , expr.toString());  //XXX                          
