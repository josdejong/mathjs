...
function factory (type, config, load, typed) {
  var isPolynomial = load(require('./isPolynomial'));
  ...
  var ut = require('./simplify/utilPoly'));
   ...
   * See also:
   *
   *     isPolynomial, numerator, denominator
   *
   * @param  {Node | string} expr       The expression to check if is polynomial expression
   * @param  {scopeAdd: object, reduce:boolean, numerator:node, denominator:node, hasZero:boolean, coef:array, varNames: boolean} [options]
   *                `scopeAdd` - Default empty. Scope of expression (Input) 
   *                `reduce` -   Default false. TRUE if cut lesser zeroed exponents in numerator (Input)
   *                `numerator` - Numerator node in a tree expression (Output)
   *                `denominator` - denominator node in a tree expression (Output)
   *                `hasZero` - Returns true if there is some exponent cut (Output)
   *                `coef`   - Array of coefficients of numerator sorted by increased exponent (Output)
   *                `varNames` - Variable names in expression (Output)
   *
...
if (typeof node==='string') node = ut.stringToNode(node);
if (typeof node==='string') return node;

if (scopeAdd ===undefined)
   scopeAdd={}
else  
   if (typeof scopeAdd !=='object') return 'Second optional parameter should be a Scope object'

if (varNames === undefined)
   varNames=[];
else  
   if (! Array.isArray(varNames)) return 'Thrid optional parameter should be a variable names array'
...
if (typeof node==='string') return node;  // return string is error. 
...
if (nVars>=1) {   // If expression in not a constant
    ...
    while (true) {                // Apply alternately  successive division rules and sum of fraction rules until no more changes
...
if (node.type==='OperatorNode'  &&  node.op==='/')  {   // Separate numerator from denominator
    if (nVars===1) node.args[0] = polyToCanonical(node.args[0],coef);
    if (nVars===1) node.args[1] = polyToCanonical(node.args[1]);
} else 
    if (nVars===1) node = polyToCanonical(node,coef);
...
if (coef===undefined)
   ...
   if (! Array.isArray(coef))  return 'Second optional parameter needs to be an array'
...
if (tp==='FunctionNode')            // ***** FunctionName *****
   ...
   throw 'There is an unsolved function call'   
  ...
  if (noPai!==null)  {
    ...
    if ((node.fn==='subtract' || node.fn==='add' || node.fn==='multiply')  && 
        noPai.fn!=='add' &&  noPai.fn!=='subtract' )
          throw 'Invalid ' + node.op +  ' placing'
     // -,+ : first child
    if ((node.fn==='subtract' || node.fn==='add' || node.fn==='unaryMinus' ) 
                                                  && o.noFil!==0 )
          throw 'Invalid ' + node.op +  ' placing'
  }     
  // ^,&,-(unary): firers
