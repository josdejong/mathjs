...
function factory (type, config, load, typed) {
  ...
  function unflattenr(node) {
    ...
    var makeNode = _createMakeNodeFunction(node);
      ...
      for (var i=0; i<ruleSet.length; i++) {
        if (typeof ruleSet[i] === 'function') {
          ...
          if (!matches && rule.l.isOperatorNode && isAssociative(rule.l)) {
            var makeNode = _createMakeNodeFunction(rule.l);
  ...
  function _createMakeNodeFunction(node) {
    if (node.isOperatorNode) {
      ...
      return function(args){
        return new FunctionNode(new Symbol(node.name), args);
      };
    }
  }
  /**
