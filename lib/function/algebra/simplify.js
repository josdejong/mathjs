...
function factory (type, config, load, typed, math) {
  ...
  var default_rules = [
    ...
    { l: "n+-n1", r:"n-n1" },
    { l: "log(e)", r:"1" },
    { l: "n1+n2-n3", r: "n1+(n2-n3)" }, // move + above -
    // regroup multiplication and division
    { l: "n1*(n2/n3)", r: "(n1*n2)/n3" }, // move / above *
    { l: "n1/n2/n3", r: "n1/(n2*n3)" },
    // { l: "(n*n1)/(n*n2)", r: "n1/n2" },

    // TODO the rules below are not really simplifying so should be applied separately
    // ordering constants for nice looking output
  ...
  var _simplify = typed('_simplify', {
    'Node, Array': function (node, ruleSet) {
      ...
      if (res instanceof OperatorNode || res instanceof FunctionNode) {
        if (res.args) {
          for(var i=0; i<res.args.length; i++) {
            res.args[i] = _simplify(res.args[i], ruleSet);
          }
        }
      }
      else if(res instanceof ParenthesisNode) {
        if(res.content) {
          res.content = _simplify(res.content, ruleSet);
        }
      }

      // Try to match a rule against this node
      for (var i=0; i<ruleSet.length; i++) {
        if (typeof ruleSet[i] === 'function') {
          unflattenl(res); // using left-heavy binary tree here since custom rules may expect it
          res = ruleSet[i](res);
        }
        else {
          res = res.clone();
          ...
          rule = {l:ruleSet[i].l, r:ruleSet[i].r};
          ...
          if (!matches && rule.l.isOperatorNode && isAssociative(rule.l)) {
            var makeNode = createMakeNodeFunction(rule.l);
            var expandsym = _getExpandPlaceholderSymbol();
            rule.l = makeNode([rule.l.clone(), expandsym]);
            // Push the expandsym into the deepest possible branch.
            // This helps to match the rule against nodes returned from getSplits() later on.
            flatten(rule.l);
            unflattenr(rule.l);
            rule.r = makeNode([rule.r, expandsym]);
            matches = _ruleMatch(rule.l, res)[0];
          }
  ...
  /**
   * Returns the set union of two matches placeholders or null if there is a conflict.
   */
