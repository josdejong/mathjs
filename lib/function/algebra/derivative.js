...
function factory (type, config, load, typed, math) {
    ...
    'FunctionNode, Object': function (node, constNodes) {
      ...
      switch (node.name) {
        ...
        case 'nthRoot':
          ...
          // Rearrange from nthRoot(x, c) -> x^(1/c)
          arg2 = new OperatorNode('/', 'divide', [
            new ConstantNode('1', config.number),
            node.args[1]
          ]);

          if (constNodes[node.args[1]] !== undefined) {
            constNodes[arg2] = true;
          }
        ...
        case 'log':
          ...
          if (!arg2 && node.args.length == 1) {
            ...
            arg2 = new OperatorNode('*', 'multiply', [
              arg1.clone(),
        ...
        case 'csc':
          ...
          return new OperatorNode('*', 'multiply', [
            new OperatorNode('-', 'unaryMinus', [_derivative(arg1, constNodes)]),
            new OperatorNode('*', 'multiply', [
              node,
        ...
        case 'sinh':
          ...
          return new OperatorNode('*', 'multiply', [
            _derivative(arg1, constNodes),
            new FunctionNode('cosh', [arg1.clone()])
          ]);
        case 'cosh':
          // d/dx(cosh(x)) = sinh(x)
          return new OperatorNode('*', 'multiply', [
            _derivative(arg1, constNodes),
            new FunctionNode('sinh', [arg1.clone()])
          ]);
        case 'tanh':
  ...
  /**
   * Ensures the number of arguments are correct, and will throw an error otherwise.
   *
   * @param {Node} node
   */
