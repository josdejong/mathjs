...
function factory (type, config, load, typed) {
   ...
   * Examples:
   *
   *     math.simplify(math.derivative('x^2', 'x')); // {2*x}
   *     math.derivative('x^2', 'x').simplify();     // {2*x}
   *     math.derivative('2*x', 'x').eval();         // 2
   *     math.derivative('x^2', 'x').eval({x: 4});   // 8
   *     var f = math.parse('x^2');
   *     var x = math.parse('x');
   *     math.derivative(f, x).simplify();           // {2*x}
   *
  ...
  var derivative = typed('derivative', {
    ...
    /* TODO: implement and test syntax with order of derivatives
