...
function factory (type, config, load, typed) {
   ...
   * Examples:
   *
   *     math.derivative('x^2', 'x');                     // {2*x}
   *     math.derivative('x^2', 'x', {simplify: false});  // {2 * 1 * x ^ (2 - 1)
   *     math.derivative('2*x', 'x').eval();              // 2
   *     math.derivative('x^2', 'x').eval({x: 4});        // 8
   *     var f = math.parse('x^2');
   *     var x = math.parse('x');
   *     math.derivative(f, x);                           // {2*x}
   *
   * @param  {ConstantNode | SymbolNode | ParenthesisNode | FunctionNode | OperatorNode} expr  The expression to differentiate
