...
function factory (type, config, load, typed) {
  var simplify = load(require('./isPolynomial'));
...
if (scopeAdd === undefined)
   ...
   if (typeof(scopeAdd) !== 'object') return 'Second optional parameter shoud be an scope object'

if (varNames===undefined)
   varNames = []
else 
   if (! Array.isArray(varNames)) return 'Third optional parameter shoud be an array of variable names'
...
if (tp==='FunctionNode') 
  ...
  if (node.op==='^')  {
    if (node.args[1].type!=='ConstantNode' ||  ! Number.isInteger(parseFloat(node.args[1].value)))
      ...
      if (oper.indexOf(node.op) === -1) throw 'Operator ' + node.op + ' invalid in polynomial expression';
