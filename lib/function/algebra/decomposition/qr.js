...
function factory (type, config, load, typed) {
  ...
  var hypot = load(require('../../arithmetic/hypot'));
  ...
  /**
   * Calculate the Matrix QR decomposition. Matrix `A` is decomposed in two matrices (`Q`, `R`) where `Q` is an 
   * orthoganal matrix and `R` is an upper triangular matrix.
   *
   * Syntax:
   *
   *    math.qr(A);
   ...
   *
   * @return {{Q: Array | Matrix, R: Array | Matrix}} Q: the orthoganal matrix and R: the upper triangular matrix
   */
  var qr = typed('qr', {
  ...
  var _denseQR = function (m) {
    ...
    for (k = 0; k < Math.min(cols, rows); ++k) {
       ...
       *  * Household matrix is a reflection in a plane normal to v which will zero 
       *    out all but the top right element in R.
       *  * Appplying reflection to both Q and R will not change product.
       *  * Repeat this process on the (1,1) minor to get R as an upper triangular matrix.
       *  * Reflections leave the magnitude of the columns of Q unchanged so Q remains othoganal.
       *
       */
      
      // TODO what is the best way to find modulus of a potentialy complex vector
      var alpha = 0.0; 
      for (i = k; i < rows; i++) {
        alpha = hypot(alpha,abs(Rdata[i][k]));
      }
      ...
      alpha = multiplyScalar(sgn, alpha);
