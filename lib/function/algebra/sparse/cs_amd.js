...
function factory (type, config, load) {
  ...
  var cs_amd = function (order, a) {
    ...
    var mark = _initializeQuotientGraph();
    
    // initialize degree lists
    var nel = _initializeDegreeLists();
    ...
    while (nel < n) {
      ...
      for (k1 = 1; k1 <= elenk + 1; k1++) {
        ...
        for (k2 = 1; k2 <= ln; k2++) {
          ...
          cindex[pk2++] = i;
          if (W[next + i] != -1) 
            W[last + W[next + i]] = W[last + i];
          // check we need to remove i from degree list
          if (W[last + i] != -1) 
            W[next + W[last + i]] = W[next + i];
          else
   ...
   *
   * Order: 1
   *   Matrix must be square. This is appropriatefor a Cholesky or LU factorization.
   *   P = M + M'
   *
   * Order: 2
