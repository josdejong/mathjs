...
module.exports = function (math) {
   ...
   * See also:
   *
   *    and, or, not, left_shift, arith_right_shift, log_right_shift
   *
   * @param  {Number | BigNumber | Boolean | Unit | String | Array | Matrix | null} x First value to xor
   * @param  {Integer | BigNumber | Boolean | Unit | String | Array | Matrix | null} y Second value to xor
  ...
  math.xor = function xor(x, y) {
    ...
    if (isString(x)) {
      return xor(parseInt(x), y);
    }
    if (isString(y)) {
      return xor(x, parseInt(y));
    }
