...
module.exports = function (math) {
   ...
   * See also:
   *
   *    or, xor, not, left_shift, arith_right_shift, log_right_shift
   *
   * @param  {Number | BigNumber | Boolean | Unit | String | Array | Matrix | null} x First value to and
   * @param  {Integer | BigNumber | Boolean | Unit | String | Array | Matrix | null} y Second value to and
  ...
  math.and = function and(x, y) {
    ...
    if (isString(x)) {
      return and(parseInt(x), y);
    }
    if (isString(y)) {
      return and(x, parseInt(y));
    }
