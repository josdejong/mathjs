...
module.exports = function (math) {
   ...
   * See also:
   *
   *    and, xor, not, left_shift, arith_right_shift, log_right_shift
   *
   * @param  {Number | BigNumber | Boolean | Unit | String | Array | Matrix | null} x First value to or
   * @param  {Integer | BigNumber | Boolean | Unit | String | Array | Matrix | null} y Second value to or
  ...
  math.or = function or(x, y) {
    ...
    if (isString(x)) {
      return or(parseInt(x), y);
    }
    if (isString(y)) {
      return or(x, parseInt(y));
    }
