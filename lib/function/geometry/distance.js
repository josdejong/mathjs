...
function factory (type, config, load, typed) {
   ...
   * The points can be passed as multiples of two(for 2D) or three(for 3D) numerical arguments, a 2x2 Matrix or as two 1D arrays.
   ...
   *    math.distance([[0,0], [4,4]], 2)                                    // Returns 5.6569
   *    math.intersect([0, 0], [10, 10], [10, 0], [0, 10], 2);              // Returns [5, 5]
   *    math.intersect([0, 0, 0], [10, 10, 0], [10, 0, 0], [0, 10, 0], 3);  // Returns [5, 5, 0]
...
function _2d(x){
  // checks if the number of arguments are even in count and are valid (should be numbers)
  return x.length%2 === 0 && typeof x[0] === 'number' && typeof x[1] === 'number';
}
