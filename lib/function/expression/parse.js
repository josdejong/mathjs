module.exports = function (math, settings) {
  var util = require('../../util/index'),
      ...
      IndexNode = require('../../expression/node/IndexNode'),
  ...
  function parseStart (scope) {
    ...
    if (token == '') {
      // empty expression
      node = new ConstantNode('undefined', 'undefined', math);
    }
    else {
  ...
  function parseAssignment (scope) {
    var name, params, ranges, paramScopes, expr;
    ...
    if (token == '=') {
      if (node instanceof SymbolNode) {
        ...
        // parse the expression, with the correct function scope
        // TODO: just use the IndexNode itself?
        getToken();
        name = node.object.name;
        ranges = node.ranges;
        paramScopes = node.paramScopes;
        expr = parseAssignment(scope);
        return new UpdateNode(math, name, ranges, paramScopes, expr, scope);
      }
      else {
  ...
  function parseRange (scope) {
    ...
    if (token == ':') {
      ...
      node = new ConstantNode('number', '1', math);
    }
    else {
      // explicit start
  ...
  function parseBitwiseConditions (scope) {
    ...
    /* TODO: implement bitwise conditions
     ...
     node = new OperatorNode(name, fn, math[fn], params);
     }
     */
  ...
  function parseUnary (scope) {
    ...
    if (token == '-') {
      ...
      return new OperatorNode(name, fn, math[fn], params);
    }
  ...
  function parseString (scope) {
    ...
    if (token == '"') {
      ...
      // create constant
      node = new ConstantNode('string', str, math);
  ...
  function parseNumber (scope) {
    ...
    if (token_type == TOKENTYPE.NUMBER) {
      ...
      if (token == 'i' || token == 'I') {
        ...
        getToken();
        node = new ConstantNode('complex', number, math);
      }
      else {
        // a number
        node = new ConstantNode('number', number, math);
      }
