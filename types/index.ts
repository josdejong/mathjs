...
const assertNotNil: <T> (value: T) => asserts value is NonNullable<T> = (value) => {
  if (value === undefined) {
...
*/
{
  ...
  const _thing = math.atan2(3, -3) / math.pi
  math.log(10000, 10)
  math.sqrt(-4)
  math.pow([[-1, 2], [3, 1]], 2)
  const angle = 0.2
...
*/
{
  ...
  node1.traverse((node: math.MathNode, _path: string, _parent: math.MathNode) => {
    switch (node.type) {
      case 'OperatorNode':
 ...
 */
{
  ...
  const createNegativeSquare = factory(name, dependencies, (injected) => {
    ...
    return function negativeSquare (x: number): number {
      assertNotNil(unaryMinus)
      assertNotNil(multiply)
  ...
  const unaryMinus = (a: number): number => -a
 ...
 */
{
  ...
  isFuncs.forEach(f => {
    const result = f(1)
    const isResultBoolean = result || !result
